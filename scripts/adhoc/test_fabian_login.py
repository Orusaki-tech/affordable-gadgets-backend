...
def test_fabian_login():
    ...
    try:
        ...
        if authenticated_user:
            ...
            print("❌ Django authentication FAILED - Invalid password")
            return False
        ...
        # Try with email
        serializer_email = AdminAuthTokenSerializer(data={"username": email, "password": password})

        if serializer_email.is_valid():
            print("✅ Serializer validation SUCCESS (with email)")
            ...
            print("❌ Serializer validation FAILED (with email)")
        ...
        if serializer_username.is_valid():
            print("✅ Serializer validation SUCCESS (with username)")
        else:
            print("❌ Serializer validation FAILED (with username)")
        ...
        try:
            ...
            print("⚠️  Token does not exist (will be created on login)")
        ...
        if user.is_staff or user.is_superuser:
            print("✅ User CAN login via AdminTokenLoginView")
            print(
                f"   Reason: User has is_staff={user.is_staff} or is_superuser={user.is_superuser}"
            )
        else:
            print("❌ User CANNOT login via AdminTokenLoginView")
            print("   Reason: User does not have is_staff=True")
            print("\n   FIX: Set is_staff=True for this user")
            print("   Run: user.is_staff = True; user.save()")
            return False
