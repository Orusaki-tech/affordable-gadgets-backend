...
from inventory.models import Brand, InventoryUnit, Product
...
for status in statuses:
    status_display = dict(InventoryUnit.SaleStatusChoices.choices).get(
        status["sale_status"], status["sale_status"]
    )
...
for product in products:
    ...
    units_filter = Q(sale_status=InventoryUnit.SaleStatusChoices.AVAILABLE, available_online=True)
    ...
    if count > 0:
        products_with_units.append({"product": product, "count": count, "units": available})
    else:
        total_units = product.inventory_units.count()
        products_without_units.append(
            {"product": product, "total_units": total_units, "available_units": available.count()}
        )
...
if products_without_units:
    for item in products_without_units[:10]:  # Show first 10
        ...
        if all_product_units.exists():
            statuses = all_product_units.values("sale_status", "available_online").annotate(
                count=Count("id")
            )
            print("   Unit statuses:")
            for status in statuses:
                ...
                print(
                    f"     - {status_display}, online={status['available_online']}: {status['count']}"
                )
        else:
...
for brand in brands:
    ...
    brand_units = available_units.filter(Q(brands=brand) | Q(brands__isnull=True))
