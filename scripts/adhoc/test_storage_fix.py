                        ...
                        if "cloudinary.com" in banner_url:
                            ...
                            try:
                                ...
                                if e.code == 404:
                                    ...
                                    print("   ❌ Current issue: File not uploaded to Cloudinary")
                                else:
                                    print(f"   ❌ Image error: HTTP {e.code}")
                        else:
                            print("   ❌ URL is NOT Cloudinary URL")
                            print("   ❌ Current issue: Using local storage")
                else:
...
def test_storage_logic():
    ...
    if not all([cloud_name, api_key, api_secret]):
        ...
        print("   (This is OK - they should be set in Render)")
        return False
    ...
    try:
        ...
        print("   ✅ MediaCloudinaryStorage can be instantiated")
        ...
        if "cloudinary" in storage_str.lower():
            print("   ✅ Storage is Cloudinary storage")
            return True
        else:
            print("   ⚠️  Storage type doesn't contain 'cloudinary'")
            print("   ⚠️  Might still fall back to local storage")
...
def test_model_storage():
    ...
    try:
        ...
        storage_str = str(type(storage))
        if storage is None:
            print("   ⚠️  Storage is None - will use DEFAULT_FILE_STORAGE")
            print("   ⚠️  This might still use FileSystemStorage")
