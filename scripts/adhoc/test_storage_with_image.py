...
is_cloudinary = (
    "cloudinary" in str(type(default_storage)).lower()
    or "cloudinary" in str(default_storage.__class__.__module__).lower()
)
if is_cloudinary:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10, context=ssl_context) as response:
                ...
                if status == 200:
                    ...
                    print("   ✅ Upload to Cloudinary worked!")
                    print("\n   🎉 SUCCESS! Images CAN be uploaded to Cloudinary!")
                    print("   Check your Cloudinary dashboard for the 'promotions' folder")
                else:
                    print(f"   ⚠️  Image returned status: {status}")
        except Exception as e:
            print(f"   ⚠️  Could not verify accessibility: {e}")
            print("   But file was saved, so upload might have worked")
    else:
