...
def test_image_url(url, description):
    ...
    try:
        ...
        return {"accessible": False, "status": None, "content_type": None, "error": str(e)}
                    ...
                    for promotion in data["results"]:
                        ...
                        if image_url:
                            ...
                            if "error" not in analysis:
                                ...
                                print(
                                    f"      Transformations: {analysis.get('transformations', 'None')}"
                                )
                                print(f"      Public ID: {analysis.get('public_id')}")
                                print(
                                    f"      Has 'media/' prefix: {'✅ YES' if analysis.get('has_media_prefix') else '❌ NO'}"
                                )
                                ...
                                if not analysis.get("has_media_prefix"):
                                    ...
                                    if result["accessible"]:
                                        print(
                                            f"      ✅ Image is accessible! (Status: {result['status']})"
                                        )
                            ...
                            if result["accessible"]:
                                ...
                                # Try with media/ prefix
                                if "/v1/" in image_url:
                                    alt_url1 = image_url.replace(
                                        "/v1/promotions/", "/v1/media/promotions/"
                                    )
                                ...
                                if "/v1/" in image_url:
                                    alt_url3 = image_url.replace(
                                        "/v1/promotions/", "/media/promotions/"
                                    )
                                    ...
                                    if result3["accessible"]:
                                        ...
                                        print(
                                            f"      ❌ With media/ and no version: {result3['error']}"
                                        )
                        else:
                            print("\n   ❌ No banner image URL")
                else:
