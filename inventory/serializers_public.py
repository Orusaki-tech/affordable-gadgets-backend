        ...
        for img in images:
            if img.image:
                ...
                if (
                    ...
                    absolute_thumbnail = request.build_absolute_uri(
                        original_url
                    )  # Same URL for thumbnail
                    ...
                    final_thumbnail = (
                        ...
                        if (
                            thumbnail_url
                            and thumbnail_url != original_url
                            and "cloudinary.com" in thumbnail_url
                        )
                        else absolute_thumbnail
                    )
                else:
...
class PublicProductSerializer(serializers.ModelSerializer):
    ...
    def get_has_active_bundle(self, obj):
        ...
        queryset = Bundle.objects.filter(
            main_product=obj, is_active=True, show_in_listings=True
        ).filter(
        ...
        for bundle in bundles:
            if bundle.pricing_mode == Bundle.PricingMode.FIXED and bundle.bundle_price is not None:
                ...
                if (
                    ...
                    discount = (base_total * Decimal(str(bundle.discount_percentage))) / Decimal(
                        "100"
                    )
                    price = max(Decimal("0.00"), base_total - discount)
                elif (
                    bundle.pricing_mode == Bundle.PricingMode.AMOUNT
                    and bundle.discount_amount is not None
                ):
    ...
    def get_min_price(self, obj):
        ...
        if (
            ...
            and getattr(obj, "min_price", None) is not None
        ):
    ...
    def get_compare_at_min_price(self, obj):
        ...
        if hasattr(obj, "available_units_list"):
            units = obj.available_units_list
            prices = [
                float(unit.compare_at_price) for unit in units if unit.compare_at_price is not None
            ]
            return min(prices) if prices else None
        if self.context.get("view_action") == "list":
            return (
                float(obj.compare_at_min_price)
                if getattr(obj, "compare_at_min_price", None) is not None
                else None
            )
    ...
    def get_compare_at_max_price(self, obj):
        ...
        if self.context.get("view_action") == "list":
            return (
                float(obj.compare_at_max_price)
                if getattr(obj, "compare_at_max_price", None) is not None
                else None
            )
    ...
    def get_primary_image(self, obj):
        ...
        if self.context.get("view_action") == "list":
            ...
            if pl and len(pl) > 0:
                ...
                primary_image = next((img for img in images if img.is_primary), None) or (
        ...
        if primary_image and primary_image.image:
            ...
            if url_cache is not None:
                cache_key = getattr(primary_image.image, "name", None) or getattr(
                    primary_image.image, "url", None
                )
...
class PublicBundleItemSerializer(serializers.ModelSerializer):
    ...
    def _get_price_range(self, product: Product):
        ...
        units = InventoryUnit.objects.filter(
            product_template=product, sale_status="AV", available_online=True
        )
        if not units.exists():
            return None, None
    ...
    def _get_items_total(self, obj):
        ...
        for item in obj.items.all():
            ...
            if min_price is None or max_price is None:
                units = InventoryUnit.objects.filter(
                    product_template=item.product, sale_status="AV", available_online=True
                )
                if units.exists():
                    prices = list(units.values_list("selling_price", flat=True))
...
class LeadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Lead
        fields = ["lead_reference", "status", "submitted_at", "total_value", "items"]
                ...
                try:
                    ...
                    if "/upload/" in original_url:
                        ...
                        if len(parts) == 2:
                            ...
                            if path_parts and (
                                "," in path_parts[0]
                                or any(x in path_parts[0] for x in ["w_", "h_", "c_", "q_", "f_"])
                            ):
                    ...
                    logger.warning(
                        f"Failed to add transformations to Cloudinary URL: {e}. Using original URL."
                    )
                    # Fall back to using get_optimized_image_url
                    cloudinary_url = get_promotion_image_url(
                        obj.banner_image, size="lg", crop="fill"
                    )
                ...
                if hasattr(obj.banner_image, "name") and obj.banner_image.name:
                    try:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to build Cloudinary URL with public_id '{public_id}': {e}"
                            )
                        ...
                        if public_id.startswith("media/"):
                            try:
                                ...
                                logger.warning(
                                    f"Failed to build Cloudinary URL without 'media/' prefix: {e}"
                                )
                        ...
                        if not public_id.startswith("media/"):
                            try:
                                ...
                                logger.warning(
                                    f"Failed to build Cloudinary URL with 'media/' prefix: {e}"
                                )
                    except Exception:
