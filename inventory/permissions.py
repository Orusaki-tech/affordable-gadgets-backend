...
class CanApproveRequests(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        if not request.user or not request.user.is_authenticated:
            logger.warning("CanApproveRequests: User not authenticated")
            return False
        ...
        if not admin:
            logger.warning(
                f"CanApproveRequests: No Admin profile found for user {request.user.username}"
            )
            return False

        is_im = admin.is_inventory_manager
        logger.info(
            f"CanApproveRequests.has_permission: User {request.user.username}, Admin {admin.admin_code}, is_inventory_manager={is_im}, roles={list(admin.roles.values_list('name', flat=True))}"
        )
        return is_im
