...
from .models import InventoryUnit, Product  # ... and your other models
...
class CartAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        t1 = time.time()
        logger.info(f"CartAnalyticsView: active_carts query took {t1-t0:.4f}s")

        # 2. Total active carts
        t0 = time.time()
        total_active_carts = active_carts.count()
        t1 = time.time()
        logger.info(f"CartAnalyticsView: total_active_carts query took {t1-t0:.4f}s")
        ...
        t1 = time.time()
        logger.info(f"CartAnalyticsView: total_items_in_carts query took {t1-t0:.4f}s")
        ...
        t1 = time.time()
        logger.info(f"CartAnalyticsView: popular_items query took {t1-t0:.4f}s")
        ...
        t1 = time.time()
        logger.info(f"CartAnalyticsView: stale_carts_qs query took {t1-t0:.4f}s")
        ...
        t1 = time.time()
        logger.info(f"CartAnalyticsView: total_stale_items query took {t1-t0:.4f}s")
        ...
        end_time = time.time()
        logger.info(f"CartAnalyticsView: total execution time {end_time-start_time:.4f}s")
...
from .serializers import (  # noqa: E402
    ...
    ReviewSerializer,
    TagSerializer,
    UnitAcquisitionSourceSerializer,
    UnitTransferSerializer,
)
...
try:
    if getattr(settings, "SILKY_ENABLED", False):
        from silk.collector import DataCollector as _SilkDataCollector  # noqa: N813
        from silk.profiling.profiler import silk_profile as _silk_profile

        _silk_data_collector = _SilkDataCollector
    else:
        _silk_profile = None
...
class ProductViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == "retrieve" and self.kwargs.get("pk"):
            ...
            available_stock_expr = Case(
                When(
                    ...
                    then=Coalesce(
                        Sum("inventory_units__quantity", filter=available_units_filter), Value(0)
                    ),
                ),
        ...
        if stock_status:
            if stock_status == "discontinued":
                ...
                queryset = queryset.filter(
                    available_stock__gt=0, available_stock__lt=F("min_stock_threshold")
                )
            elif stock_status == "in_stock":
                queryset = queryset.filter(is_discontinued=False, available_stock__gt=0)
                queryset = queryset.filter(
                    Q(min_stock_threshold__isnull=True)
                    | Q(available_stock__gte=F("min_stock_threshold"))
                )
        ...
        if seo_status:
            ...
            seo_score_points = (
                ...
                + Case(
                    When(
                        Q(meta_description__isnull=False) & ~Q(meta_description=""), then=Value(1)
                    ),
                    default=Value(0),
                    output_field=IntegerField(),
                )
                ...
                + Case(
                    When(
                        Q(product_description__isnull=False) & ~Q(product_description=""),
                        then=Value(1),
                    ),
                    default=Value(0),
                    output_field=IntegerField(),
                ...
                + Case(
                    When(
                        Q(product_highlights__isnull=False) & ~Q(product_highlights=[]),
                        then=Value(1),
                    ),
                    default=Value(0),
                    output_field=IntegerField(),
            ...
            queryset = queryset.annotate(
                seo_score=ExpressionWrapper(
                    seo_score_points * Value(100.0) / Value(8.0), output_field=FloatField()
                )
            )
    ...
    def perform_create(self, serializer):
        ...
        if admin and admin.brands.exists() and not admin.is_global_admin:
            ...
            if not brand_ids_provided:
                ...
                isinstance(self.request.data.get("brand_ids"), list)
                and len(self.request.data.get("brand_ids", [])) == 0
            ):
        ...
        if not product_instance.brands.exists():
            try:
                ...
                default_brand = Brand.objects.filter(
                    code="AFFORDABLE_GADGETS", is_active=True
                ).first()
                if default_brand:
                    product_instance.brands.set([default_brand])
    ...
    def perform_destroy(self, instance):
        ...
        available_units_count = (
            available_units.aggregate(total=Coalesce(Sum("quantity"), Value(0)))["total"] or 0
        )
        ...
        try:
            ...
            logger.error(
                f"Error cleaning related data for product {instance.id}: {str(e)}", exc_info=True
            )
            from rest_framework.exceptions import ValidationError
    ...
    def bulk_destroy(self, request):
        ...
        if not IsInventoryManagerOrSuperuser().has_permission(request, self):
            return Response(
                {"detail": "Only Inventory Managers and Superusers can bulk-delete products."},
    ...
    def available_units(self, request, pk=None):
        ...
        product = self.get_object()
        qs = (
            product.inventory_units.filter(sale_status=InventoryUnit.SaleStatusChoices.AVAILABLE)
...
class InventoryUnitViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    filterset_fields = {
        ...
        "selling_price": [
            ...
            "gte",
            "exact",
        ],  # Filter by price range (less than/greater than or equal to)
    ...
    def approve_buyback(self, request, pk=None):
        ...
        if unit.source != InventoryUnit.SourceChoices.BUYBACK_CUSTOMER:
            return Response(
                {"error": "Only buyback items can be approved."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"message": "Buyback item approved and made available.", "unit": serializer.data}
        )
    ...
    def create_order(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": f"Unit not found: {str(e)}"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        try:
            with transaction.atomic():
                ...
                try:
                    from inventory.observability import NEW_ORDERS_TOTAL
    ...
    def export_csv(self, request):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="inventory_units_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
    ...
    def import_csv(self, request):
        ...
        try:
            ...
            return Response(
                {"error": f"Encoding error 1 : {str(e)}"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def bulk_update(self, request):
        ...
        if not unit_ids or not isinstance(unit_ids, list):
            return Response(
                {"error": "unit_ids must be a non-empty list"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            with transaction.atomic():
                if operation == "update_price":
                    ...
                    # Validate status
                    valid_statuses = [
                        choice[0] for choice in InventoryUnit.SaleStatusChoices.choices
                    ]
...
class PhoneSearchByBudgetView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.select_related("product_template", "product_color").order_by(
            "selling_price"
...
class PublicAvailableUnitsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        qs = (
            InventoryUnit.objects.filter(sale_status=InventoryUnit.SaleStatusChoices.AVAILABLE)
            .select_related("product_template", "product_color")
            .order_by("selling_price")
        )
...
class ReviewViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    queryset = (
        Review.objects.all().select_related("product", "customer__user").order_by("-date_posted")
    )
        ...
        if self.request.user.is_staff:
            ...
            try:
                ...
                raise exceptions.PermissionDenied(
                    "Review creation requires a valid Customer profile."
                )
    ...
    def bulk_action(self, request):
        ...
        if not action_type or not review_ids:
            return Response(
                {"error": "action and review_ids are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if action_type == "delete":
            ...
            return Response(
                {"error": f"Unknown action: {action_type}"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class AdminViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    def assign_roles(self, request, pk=None):
        ...
        if not isinstance(role_ids, list):
            return Response(
                {"error": "role_ids must be a list"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def assign_brands(self, request, pk=None):
        ...
        if not isinstance(brand_ids, list):
            return Response(
                {"error": "brand_ids must be a list"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def perform_destroy(self, instance):
        ...
        logger.info(
            f"Deleting admin {admin_code} and associated user {user.username if user else 'None'}"
        )
        ...
        if user:
            try:
                ...
                if has_customer:
                    ...
                    user.delete()
                    logger.info(
                        f"Successfully deleted user {user.username} associated with admin {admin_code}"
                    )
            except Exception as e:
...
class OrderViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Order.objects.all()
            .order_by("-created_at")
            .select_related("customer__user", "user", "brand")
        )
    ...
    def get_object(self):
        ...
        lookup_value = (
            self.kwargs.get(lookup_url_kwarg)
            or self.kwargs.get("pk")
            or self.kwargs.get("order_id")
            or self.kwargs.get(self.lookup_field)
        )
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {
                                ...
                                "resolver_match_route": self.request.resolver_match.route
                                if hasattr(self.request, "resolver_match")
                                and self.request.resolver_match
                                else None,
                                "resolver_match_url_name": self.request.resolver_match.url_name
                                if hasattr(self.request, "resolver_match")
                                and self.request.resolver_match
                                else None,
                                "resolver_match_kwargs": dict(self.request.resolver_match.kwargs)
                                if hasattr(self.request, "resolver_match")
                                and self.request.resolver_match
                                else None,
                            },
        ...
        print(
            f"[GET_OBJECT] Full URL: {self.request.build_absolute_uri() if hasattr(self.request, 'build_absolute_uri') else 'N/A'}"
        )
        ...
        if hasattr(self.request, "resolver_match") and self.request.resolver_match:
            ...
            if "receipt" in route or "receipt" in url_name:
                ...
                self.action = "receipt"
                print("[GET_OBJECT] DETECTED receipt route, forcing action to 'receipt'")

        # Also check the path directly as fallback
        if not is_receipt_request and "receipt" in self.request.path:
            is_receipt_request = True
            self.action = "receipt"
            print("[GET_OBJECT] DETECTED 'receipt' in path, forcing action to 'receipt'")
            print(
                "[GET_OBJECT] WARNING: Resolver route doesn't match receipt, but path contains 'receipt'"
            )
            print(
                "[GET_OBJECT] This suggests a routing issue - receipt route may not be registered correctly"
            )

        is_receipt_or_retrieve = is_receipt_request or getattr(self, "action", None) in [
            "receipt",
            "retrieve",
        ]
            ...
            if "orders" in path_parts:
                ...
                if orders_index + 1 < len(path_parts):
                    ...
                    if len(potential_order_id) >= 32:  # Accept both formats
                        ...
                        print(
                            f"[GET_OBJECT] Extracted lookup_value from path for receipt/retrieve: {lookup_value}"
                        )
        ...
        if self.lookup_field == "order_id" and lookup_value:
            try:
                ...
                print(
                    f"[GET_OBJECT] Attempting direct lookup for order_id: {lookup_value} (type: {type(lookup_value).__name__})"
                )
                # #region agent log
                try:
                ...
                # #endregion
                logger.info(
                    "Order found via get_object() direct lookup",
                    extra={
                        "order_id": str(order.order_id),
                        "order_status": order.status,
                ...
                # #endregion
                logger.error(
                    f"Order not found in get_object(): {lookup_value}",
                    extra={
        ...
        print("[GET_OBJECT] Using default get_object() behavior")
        try:
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        # #region agent log
        logger.info(
            "DEBUG: OrderViewSet.retrieve() called",
            extra={
                "hypothesisId": "B",
                ...
                "resolver_match_route": request.resolver_match.route
                if hasattr(request, "resolver_match") and request.resolver_match
                else None,
                "resolver_match_url_name": request.resolver_match.url_name
                if hasattr(request, "resolver_match") and request.resolver_match
                else None,
            },
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        # #region agent log
        logger.info(
            "Idempotency key extracted from header",
            extra={
                "idempotency_key_present": bool(idempotency_key),
                "idempotency_key_preview": idempotency_key[:20] + "..."
                if idempotency_key
                else None,
            },
        )
        ...
        if idempotency_key:
            ...
            try:
                ...
                try:
                    ...
                    column_exists = False
                    logger.warning(
                        f"Could not check if idempotency_key column exists: {str(db_check_error)}"
                    )
                ...
                if not column_exists:
                    ...
                    logger.warning(
                        "idempotency_key column does not exist - migration may not have been run. Proceeding without idempotency check."
                    )
                else:
    ...
    def perform_create(self, serializer):
        ...
        idempotency_key = self.request.headers.get("Idempotency-Key") or self.request.headers.get(
            "X-Idempotency-Key"
        )
        ...
        admin = (
            get_admin_from_user(self.request.user)
            if self.request.user.is_authenticated and self.request.user.is_staff
            else None
        )
        ...
        if self.request.user.is_authenticated and self.request.user.is_staff:
            ...
            if not admin:
                raise exceptions.PermissionDenied("Staff account is missing an admin profile.")
            is_walk_in_creator = self.request.user.is_superuser or (
        ...
        if brand_id:
            ...
            if self.request.user.is_authenticated and self.request.user.is_staff:
                ...
                if not (self.request.user.is_superuser or admin.is_global_admin):
                    if admin.brands.exists():
                        ...
                        raise exceptions.PermissionDenied(
                            "You must be assigned to at least one brand to create orders."
                        )
            ...
            if order_source == Order.OrderSourceChoices.WALK_IN:
                if admin.brands.count() == 1:
                    ...
                    raise exceptions.ValidationError(
                        {"brand": "Brand is required for walk-in orders created by staff."}
                    )
        ...
        try:
            ...
            logger.info(
                f"Order saved successfully: {serializer.instance.order_id if serializer.instance else 'N/A'}"
            )
        except Exception as e:
            # Log the full error with traceback - this will show up in Render logs
            logger.error(
                ...
                for order_item in instance.order_items.all():
                    if order_item.inventory_unit:
                        ...
                        if unit.sale_status in [
                            ...
                            unit.save(
                                update_fields=["sale_status", "reserved_by", "reserved_until"]
                            )
        ...
        try:
            ...
            if admin.is_salesperson and not admin.is_global_admin:
                ...
                if order.brand and order.brand not in admin.brands.all():
                    raise exceptions.PermissionDenied(
                        "You can only confirm payment for orders from your assigned brands."
                    )
        except Admin.DoesNotExist:
            pass
                ...
                if unit.product_template.product_type == Product.ProductType.ACCESSORY:
                    ...
                    if admin:
                        ...
                        for req in reservation_requests:
                            ...
                            req.save(
                                update_fields=["inventory_unit_quantities", "status", "expires_at"]
                            )
            ...
            try:
                if hasattr(order, "source_lead") and order.source_lead:
                    ...
                    if hasattr(lead, "cart") and lead.cart:
                        ...
                        cart_cleared = True
                        print(
                            f"Cart {cart.id} deleted after payment confirmed for order {order.order_id}"
                        )
            except Exception as e:
    ...
    def initiate_payment(self, request, pk=None, **kwargs):
        ...
        try:
            ...
            print("[PESAPAL] Getting order object...")
            order = self.get_object()
            print(
                f"[PESAPAL] Order retrieved - ID: {order.order_id}, Status: {order.status}, Amount: {order.total_amount}"
            )
            ...
            if not callback_url:
                ...
                if not callback_url:
                    print("[PESAPAL] ========== VIEW: INITIATE PAYMENT FAILED ==========")
                    print("[PESAPAL] ERROR: callback_url is required")
                    print("[PESAPAL] ==================================================\n")
                    return Response(
                        {"error": "callback_url is required"}, status=status.HTTP_400_BAD_REQUEST
                    )
            ...
            if not billing_address and order.customer and order.customer.delivery_address:
                billing_address = {"line_1": order.customer.delivery_address}

            print("[PESAPAL] Calling service.initiate_payment...")
            ...
            if not result.get("success"):
                print("[PESAPAL] ========== VIEW: INITIATE PAYMENT FAILED ==========")
                print("[PESAPAL] Service returned failure")
                print("[PESAPAL] ==================================================\n")
                return Response(result, status=status.HTTP_400_BAD_REQUEST)

            print("[PESAPAL] ========== VIEW: INITIATE PAYMENT SUCCESS ==========")
            print("[PESAPAL] Returning success response to client")
            print(
                f"[PESAPAL] Response includes redirect_url: {result.get('redirect_url', 'NOT PROVIDED')}"
            )
            print("[PESAPAL] ⚠️  FRONTEND MUST REDIRECT USER TO redirect_url IMMEDIATELY")
            print("[PESAPAL] ===================================================\n")
            ...
            print("[PESAPAL] ========== VIEW: INITIATE PAYMENT EXCEPTION ==========")
            ...
            return Response(
                {
                    ...
                    "detail": str(e)
                    if settings.DEBUG
                    else "An error occurred while initiating payment. Please try again or contact support.",
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def payment_status(self, request, pk=None, **kwargs):
        ...
        try:
            ...
            print("[PESAPAL] Calling service.get_payment_status...")
...
def _product_accessory_queryset():
    ...
    pending_order_items = OrderItem.objects.filter(order__status__in=pending_statuses).only(
...
class CustomerEmailVerificationView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not customer.email_verification_token or str(customer.email_verification_token) != str(
            token
        ):
            return Response(
        ...
        if customer.email_verification_sent_at:
            ...
            if timezone.now() > expiry:
                return Response(
                    {"error": "Verification link has expired."}, status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def post(self, request):
        try:
            ...
            if request.auth:
                ...
                return Response(
                    {"detail": "Successfully logged out. Token deleted."}, status=status.HTTP_200_OK
                )
            else:
                # Should not be reached if IsAuthenticated is working, but safe check.
                return Response(
                    {"detail": "Logout attempted without an active token."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        try:
            ...
            if self.request.user.is_staff or self.request.user.is_superuser:
                ...
                raise exceptions.NotFound(
                    "Admin profile not found for the authenticated admin user."
                )
            ...
            if format_type == "pdf":
                ...
                if not receipt.pdf_file or file_missing:
                    ...
                    pdf_path = os.path.join(
                        "receipts", timezone.now().strftime("%Y/%m"), pdf_filename
                    )
...
class ReservationRequestViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if admin.is_salesperson:
            return (
                ReservationRequest.objects.filter(requesting_salesperson=admin)
                .select_related(
                    "requesting_salesperson__user",
                    "inventory_unit__product_template",
                    "approved_by__user",
            ...
            if not new_status or new_status == request_obj.status:
                if request_obj.status == ReservationRequest.StatusChoices.PENDING:
                    ...
                    try:
                        serializer.save()
                        logger.info(
                            f"Successfully saved edit to reservation request {request_obj.id}"
                        )
                        return
                    except Exception as e:
                        import traceback
                        ...
                        logger.error(
                            f"Error saving edit to reservation request {request_obj.id}: {str(e)}\n{traceback.format_exc()}"
                        )
                        raise
            ...
            if new_status == ReservationRequest.StatusChoices.APPROVED:
                ...
                try:
                    ...
                    if not approving_admin.is_inventory_manager:
                        logger.warning(
                            f"User {self.request.user.username} attempted to approve but is not an inventory manager"
                        )
                        raise exceptions.PermissionDenied(
                            "Only Inventory Managers can approve reservation requests."
                        )
                except Admin.DoesNotExist:
                ...
                if request_obj.inventory_unit:
                    ...
                    other_active_approvals = (
                        other_active_approvals | other_active_approvals_old
                    ).distinct()
                ...
                if other_active_approvals.exists():
                    logger.info(
                        ...
                        extra={
                            ...
                            "other_request_ids": list(
                                other_active_approvals.values_list("id", flat=True)
                            ),
                        },
                    )
                    ...
                    for expired_req in other_active_approvals:
                        ...
                        for unit in expired_units:
                            if unit.reserved_by == expired_req.requesting_salesperson:
                                ...
                                unit.sale_status = InventoryUnit.SaleStatusChoices.AVAILABLE
                                unit.save(
                                    update_fields=["reserved_by", "reserved_until", "sale_status"]
                                )
                ...
                request_obj.save(
                    update_fields=["approved_by", "approved_at", "expires_at", "status"]
                )
                ...
                for unit in units:
                    requested_qty = (
                        unit_quantities.get(str(unit.id)) or unit_quantities.get(unit.id) or 1
                    )
                ...
                if len(unit_names) == 1:
                    ...
                    message = (
                        f"Your reservation request for {len(unit_names)} units has been approved."
                    )
                ...
                try:
                    ...
                    logger.info(
                        f"Created approval notification for salesperson {request_obj.requesting_salesperson.user.username} for request {request_obj.id}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create approval notification for salesperson {request_obj.requesting_salesperson.user.username} for request {request_obj.id}: {str(e)}"
                    )
                    # Don't raise - notification failure shouldn't break approval

                # Notify inventory managers and superusers
                approval_message = (
                    f"Reservation for {len(unit_names)} unit(s) has been approved."
                    if len(unit_names) > 1
                    else f"Reservation for {unit_names[0]} has been approved."
                )
                ...
                try:
                    ...
                    for manager in managers:
                        try:
                            ...
                            logger.error(
                                f"Failed to create notification for manager {manager.user.username}: {str(e)}"
                            )
                    ...
                    for superuser in superusers:
                        try:
                            ...
                            logger.error(
                                f"Failed to create notification for superuser {superuser.username}: {str(e)}"
                            )
                except Exception as e:
                    logger.error(
                        f"Error creating manager/superuser notifications for request {request_obj.id}: {str(e)}"
                    )
                    # Don't raise - notification failure shouldn't break approval
                ...
                try:
                    ...
                    if not approving_admin.is_inventory_manager:
                        logger.warning(
                            f"User {self.request.user.username} attempted to reject but is not an inventory manager"
                        )
                        raise exceptions.PermissionDenied(
                            "Only Inventory Managers can reject reservation requests."
                        )
                except Admin.DoesNotExist:
                    logger.error(f"Admin profile not found for user {self.request.user.username}")
                    raise exceptions.PermissionDenied("Admin profile required.")
                ...
                rejection_message = (
                    f"Your reservation request for {len(unit_names)} unit(s) has been rejected."
                    if len(unit_names) > 1
                    else f"Your reservation request for {unit_names[0]} has been rejected."
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Created rejection notification for salesperson {request_obj.requesting_salesperson.user.username} for request {request_obj.id}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create rejection notification for salesperson {request_obj.requesting_salesperson.user.username} for request {request_obj.id}: {str(e)}"
                    )
...
class ReturnRequestViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            if admin.is_inventory_manager:
                ...
                queryset = ReturnRequest.objects.filter(requesting_salesperson=admin)
            else:
                return ReturnRequest.objects.none()
        ...
        _units_prefetch = Prefetch(
            "inventory_units",
            queryset=InventoryUnit.objects.select_related("product_template").prefetch_related(
                _transfers_prefetch
            ),
        )
    ...
    def _get_net_holdings_for_admins(self, admin_ids):
        ...
        return {
            aid: {
                ...
                "net_holdings": directly_reserved.get(aid, 0)
                + received.get(aid, 0)
                - transferred.get(aid, 0),
            }
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            admin_ids = list(
                {r.requesting_salesperson_id for r in page if r.requesting_salesperson_id}
            )
            context = self.get_serializer_context()
            context["net_holdings_by_admin_id"] = (
                self._get_net_holdings_for_admins(admin_ids) if admin_ids else {}
            )
            serializer = self.get_serializer(page, many=True, context=context)
            return self.get_paginated_response(serializer.data)
        admin_ids = list(
            {r.requesting_salesperson_id for r in queryset if r.requesting_salesperson_id}
        )
        context = self.get_serializer_context()
        context["net_holdings_by_admin_id"] = (
            self._get_net_holdings_for_admins(admin_ids) if admin_ids else {}
    ...
    def perform_update(self, serializer):
        ...
        if new_status == ReturnRequest.StatusChoices.APPROVED:
            ...
            serializer.save(
                approved_by=approving_admin, approved_at=timezone.now(), status=new_status
            )
            ...
            for unit in units:
                if unit.product_template.product_type == Product.ProductType.ACCESSORY:
                    ...
                    unit.reserved_until = None
                    unit.save(
                        update_fields=["quantity", "sale_status", "reserved_by", "reserved_until"]
                    )
                else:
                    if unit.sale_status == InventoryUnit.SaleStatusChoices.RESERVED:
            ...
            ReservationRequest.objects.filter(
                requesting_salesperson=request_obj.requesting_salesperson,
                status=ReservationRequest.StatusChoices.APPROVED,
            ).filter(Q(inventory_units__in=units) | Q(inventory_unit__in=units)).update(
                status=ReservationRequest.StatusChoices.RETURNED, expires_at=timezone.now()
            )
    ...
    def bulk_approve(self, request):
        ...
        with transaction.atomic():
            requests = ReturnRequest.objects.filter(
                id__in=request_ids, status=ReturnRequest.StatusChoices.PENDING
            )
        ...
        return Response(
            {"message": f"{approved_count} return requests approved"}, status=status.HTTP_200_OK
        )
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            return UnitTransfer.objects.all().select_related(
                "inventory_unit__product_template",
                "from_salesperson__user",
                "to_salesperson__user",
                "approved_by__user",
            )
...
class StockAlertsViewSet(_SilkProfileMixin, viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        low_stock_products = Product.objects.annotate(
            available_count=Case(
                When(
                    ...
                    then=Coalesce(
                        Sum(
                            "inventory_units__quantity", filter=Q(inventory_units__sale_status="AV")
                        ),
                        Value(0),
                    ),
                ),
        ...
        try:
            ...
            accessory_product = low_stock_products.filter(
                product_type=Product.ProductType.ACCESSORY
            ).first()
...
class LeadViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Lead.objects.all()
            .select_related("customer", "brand", "assigned_salesperson__user", "order")
    ...
    def assign(self, request, pk=None):
        ...
        try:
                        ...
                        json.dumps(
                            {
                                ...
                                "data": {
                                    ...
                                    "admin_brand_ids": list(
                                        admin.brands.values_list("id", flat=True)
                                    ),
                                },
                                "timestamp": int(__import__("time").time() * 1000),
                            }
            ...
            # endregion
            return Response(
                {"error": "Admin profile not found"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def convert(self, request, pk=None):
        ...
        try:
            ...
            if lead.status != Lead.StatusChoices.CONTACTED:
                return Response(
                    {"error": "Lead must be contacted first"}, status=status.HTTP_400_BAD_REQUEST
                )

            order = LeadService.convert_lead_to_order(lead, admin)
            return Response({"message": "Lead converted to order", "order_id": str(order.order_id)})
        except Admin.DoesNotExist:
            return Response(
                {"error": "Admin profile not found"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def close(self, request, pk=None):
        ...
        try:
            ...
            if not admin.is_salesperson and not request.user.is_superuser:
                return Response(
                    {"error": "Only salespersons can close leads"}, status=status.HTTP_403_FORBIDDEN
                )
    ...
    def perform_create(self, serializer):
        ...
        if discount_percentage and discount_amount:
            ...
            raise ValidationError(
                {
                    "non_field_errors": [
                        "Cannot use both discount_percentage and discount_amount. Use one or the other."
                    ]
                }
            )
        ...
        if start_date and end_date:
            ...
            if start and end and start >= end:
                ...
                raise ValidationError({"non_field_errors": ["Start date must be before end date."]})
        ...
        if not isinstance(display_locations, list):
            ...
            raise ValidationError({"display_locations": ["Display locations must be a list."]})
        ...
        if invalid_locations:
            ...
            raise ValidationError(
                {
                    "display_locations": [
                        f"Invalid location(s): {', '.join(invalid_locations)}. Valid options: {', '.join(valid_locations)}."
                    ]
                }
            )
        ...
        if not brand_id and admin and admin.brands.exists() and not admin.is_global_admin:
            ...
            if admin.brands.count() == 1:
                ...
                raise ValidationError(
                    {
                        "brand": [
                            "You are assigned to multiple brands. Please select which brand this promotion is for."
                        ]
                    }
                )
        ...
        try:
            with open(
                ...
                f.write(
                    json.dumps(
                        {
                            "location": "views.py:4200",
                            "message": "Before promotion save - checking storage and banner_image",
                            "data": {
                                "storage_type": storage_type,
                                "is_cloudinary": "cloudinary" in storage_type.lower(),
                                "has_banner_image": has_banner,
                                "banner_image_type": str(type(banner_file))
                                if banner_file
                                else None,
        ...
        if promotion_instance.banner_image:
            ...
            storage_type = str(type(default_storage))
            logger.info(
                f"DEBUG: Banner image after save - URL: {banner_url}, Name: {banner_name}, IsCloudinary: {is_cloudinary}, Storage: {storage_type}"
            )
            print(
                f"DEBUG: Banner image after save - URL: {banner_url}, Name: {banner_name}, IsCloudinary: {is_cloudinary}, Storage: {storage_type}"
            )
        ...
        try:
            with open(
                ...
                banner_url = (
                    promotion_instance.banner_image.url if promotion_instance.banner_image else None
                )
                banner_name = (
                    promotion_instance.banner_image.name
                    if promotion_instance.banner_image
                    else None
                )
                f.write(
                    json.dumps(
                        {
                            "location": "views.py:4203",
                            "message": "After promotion save - banner_image URL",
                            "data": {
                                "promotion_id": promotion_instance.id,
                                "banner_image_url": banner_url,
                                "banner_image_name": banner_name,
                                "is_cloudinary_url": "cloudinary.com" in str(banner_url).lower()
                                if banner_url
                                else False,
                            },
                            "timestamp": int(__import__("time").time() * 1000),
        ...
        if "products" in self.request.data:
            ...
            if hasattr(self.request.data, "getlist"):
                ...
                product_ids = [
                    int(p) if isinstance(p, (int, str)) else (p.id if hasattr(p, "id") else p)
                    for p in products
                    if p
                ]
            elif products:
        ...
        if promotion_code:
            if (
                Promotion.objects.filter(promotion_code=promotion_code)
                .exclude(id=promotion_instance.id)
                .exists()
            ):
                from rest_framework.exceptions import ValidationError
        ...
        try:
            with open(
                ...
                f.write(
                    json.dumps(
                        {
                            "location": "views.py:4258",
                            "message": "Before promotion update - checking storage and banner_image",
                            "data": {
        ...
        if user.is_superuser:
            ...
            try:
                with open(
                    ...
                    banner_url = (
                        promotion_instance.banner_image.url
                        if promotion_instance.banner_image
                        else None
                    )
                    banner_name = (
                        ...
                        json.dumps(
                            {
                                ...
                                "message": "After promotion update - banner_image URL",
                                "data": {
                                    "promotion_id": promotion_instance.id,
                                    "banner_image_url": banner_url,
                                    "banner_image_name": banner_name,
    ...
    def post(self, request):
        ...
        try:
            ...
            updated = InventoryUnit.objects.filter(
                ~Q(sale_status=InventoryUnit.SaleStatusChoices.AVAILABLE)
                | Q(available_online=False)
            ).update(sale_status=InventoryUnit.SaleStatusChoices.AVAILABLE, available_online=True)
...
class BundleViewSet(_SilkProfileMixin, viewsets.ModelViewSet):
    ...
    queryset = (
        Bundle.objects.all().select_related("brand", "main_product").prefetch_related("items")
    )
    ...
    def get(self, request):
        ...
        try:
            ...
            if not order_tracking_id:
                print("[PESAPAL] ========== VIEW: IPN CALLBACK FAILED ==========")
                print("[PESAPAL] ERROR: OrderTrackingId required")
                print("[PESAPAL] =============================================\n")
            ...
            print("[PESAPAL] Calling service.handle_ipn...")
            ...
            print("[PESAPAL] ========== VIEW: IPN CALLBACK SUCCESS ==========\n")
            return Response(
                {"status": "success", "message": "IPN processed"}, status=status.HTTP_200_OK
            )

        except Exception as e:
            print("[PESAPAL] ========== VIEW: IPN CALLBACK EXCEPTION ==========")
            print(f"[PESAPAL] ERROR: {str(e)}")
            import traceback
