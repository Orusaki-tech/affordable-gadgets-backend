...
from rest_framework import exceptions, filters, generics, permissions, status, viewsets
...
from inventory.serializers_public import (
    ...
    PublicWishlistItemSerializer,
    ReviewEligibilityItemSerializer,
...
class PublicProductViewSet(_PublicAPIMixin, _SilkProfileMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def brands(self, request):
        ...
        brand_rows = (
            qs.values_list("product_type", "brand").distinct().order_by("product_type", "brand")
        )
    ...
    def _log_all_products_debug(self, debug_enabled=False):
        ...
        try:
            ...
            all_products = Product.objects.all().values(
                "id", "product_name", "is_published", "is_discontinued", "is_global"
            )
            ...
            for p in all_products_list[:10]:  # Limit to first 10 for performance
                ...
                available_online_units = product_obj.inventory_units.filter(
                    sale_status=InventoryUnit.SaleStatusChoices.AVAILABLE, available_online=True
                ).count()
                ...
                if total_units > 0:
                    ...
                    units_by_status = product_obj.inventory_units.values(
                        "sale_status", "available_online"
                    ).annotate(count=Count("id"))
                    unit_status_breakdown = {
                        f"{u['sale_status']}_online_{u['available_online']}": u["count"]
                        for u in units_by_status
                    }
                    # Also get sample unit details
                    sample_units = list(
                        product_obj.inventory_units.values("id", "sale_status", "available_online")[
                            :3
                        ]
                    )
                ...
                try:
                    ...
                    try:
                        sample_products = list(
                            queryset.values("id", "product_name", "is_global")[:3]
                        )
                            ...
                            json.dumps(
                                {
                                    ...
                                    "data": {
                                        ...
                                        if not queryset_evaluates
                                        else None,
                                        "query_params": dict(request.query_params),
                                        "page": request.query_params.get("page", "1"),
                                        "page_size": request.query_params.get("page_size", "24"),
                                        "brand_code": request.headers.get(
                                            "X-Brand-Code", "NOT_PROVIDED"
                                        ),
                                    },
                                    "timestamp": int(time.time() * 1000),
                                }
                            ...
                            json.dumps(
                                {
                                    ...
                                    "data": {
                                        "response_count": response_data.get("count", 0)
                                        if response_data
                                        else 0,
    ...
    def _get_public_product_detail_queryset(self, pk):
        ...
        available_units_prefetch = Prefetch(
            "inventory_units",
            queryset=InventoryUnit.objects.filter(available_units_filter).select_related(
                "product_color"
            ),
    ...
    def get_queryset(self):
                        ...
                        json.dumps(
                            {
                                ...
                                "data": {
                                    ...
                                    "brand_code": self.request.headers.get(
                                        "X-Brand-Code", "NOT_PROVIDED"
                                    ),
        ...
        try:
            ...
            if slug:
                ...
                unit_count_sub = (
                    unit_base.values("product_template")
                ...
                compare_min_sub = compare_unit_base.order_by("compare_at_price").values(
                    "compare_at_price"
                )[:1]
                compare_max_sub = compare_unit_base.order_by("-compare_at_price").values(
                    "compare_at_price"
                )[:1]
                review_base = Review.objects.filter(product=OuterRef("pk"))
                review_count_sub = (
                    review_base.values("product").annotate(total=Count("id")).values("total")[:1]
                )
                average_rating_sub = (
                    review_base.values("product").annotate(avg=Avg("rating")).values("avg")[:1]
                )
                ...
                queryset = queryset.annotate(
                    ...
                    min_price=Coalesce(
                        Subquery(min_price_sub),
                        Value(None),
                        output_field=DecimalField(max_digits=10, decimal_places=2),
                    ),
                    max_price=Coalesce(
                        Subquery(max_price_sub),
                        Value(None),
                        output_field=DecimalField(max_digits=10, decimal_places=2),
                    ),
                    compare_at_min_price=Coalesce(
                        Subquery(compare_min_sub),
                        Value(None),
                        output_field=DecimalField(max_digits=10, decimal_places=2),
                    ),
                    compare_at_max_price=Coalesce(
                        Subquery(compare_max_sub),
                        Value(None),
                        output_field=DecimalField(max_digits=10, decimal_places=2),
                    ),
                    review_count=Coalesce(
                        Subquery(review_count_sub), Value(0), output_field=IntegerField()
                    ),
                    average_rating=Coalesce(
                        Subquery(average_rating_sub),
                        Value(None),
                ...
                try:
                    ...
                    os.makedirs("/tmp/affordable-gadgets-debug", exist_ok=True)
                    accessory_sample = queryset.filter(
                        product_type=Product.ProductType.ACCESSORY
                    ).first()
                    if accessory_sample:
                        accessory_units = accessory_sample.inventory_units.filter(
                            available_units_filter
                        )
                        sum_qty = (
                            accessory_units.aggregate(total_qty=Sum("quantity"))["total_qty"] or 0
                        )
            ...
            if _run_debug_checks:
                try:
                    ...
                    try:
                        sample_products = list(
                            queryset.values(
                                "id", "product_name", "is_published", "is_discontinued"
                            )[:5]
                        )
                    except Exception:
            ...
            if _run_debug_checks:
                ...
                try:
                    logger.info(
                        f"PublicProductViewSet: brand={brand}, initial queryset exists={queryset.exists()}"
                    )
                    ...
                    if sample:
                        ...
                        logger.info(
                            f"Sample product {sample.id} ({sample.product_name}): total_units={total_units}, available={available_units}, available_online={available_online_units}"
                        )
                except Exception as e:
                    logger.warning(
                        f"PublicProductViewSet: brand={brand}, could not check queryset: {e}"
                    )
            ...
            if is_list:
                ...
                bundles_qs = (
                    Bundle.objects.filter(is_active=True, show_in_listings=True)
                    .filter(*bundle_date_filter)
                    .prefetch_related("items__product")
                )
                if brand:
                    bundles_qs = bundles_qs.filter(brand=brand)
                bundles_prefetch = Prefetch(
                    "bundles", queryset=bundles_qs, to_attr="active_bundles_list"
                )
            ...
            queryset = queryset.prefetch_related(available_units_prefetch, primary_images_prefetch)
            ...
            try:
                ...
                if _run_debug_checks:
                    try:
                        ...
                        if sample_before:
                            ...
                            if brand:
                                matching_units = matching_units.filter(
                            ...
                            if sample_before.product_type == Product.ProductType.ACCESSORY:
                                matching_qty = (
                                    matching_units.aggregate(total=Sum("quantity"))["total"] or 0
                                )
                            else:
                ...
                if _run_debug_checks and settings.DEBUG:
                    ...
                    try:
                        ...
                        logger.info(
                            f"QUERYSET_COUNT: Queryset has {count} products after prefetch (no annotations)"
                        )
                        if count > 0:
                            ...
                            if sample:
                                p = list(sample)[0]
                                logger.info(
                                    f"QUERYSET_SAMPLE: Product {p['id']} ({p['product_name']})"
                                )
                    except Exception as e:
                ...
                if _run_debug_checks and settings.DEBUG:
                    try:
                        ...
                        if sample_after:
                            prefetched_count = (
                                len(sample_after.available_units_list)
                                if hasattr(sample_after, "available_units_list")
                                else 0
                            )
                            os.makedirs("/tmp/affordable-gadgets-debug", exist_ok=True)
                            with open("/tmp/affordable-gadgets-debug/debug.log", "a") as f:
            ...
            if _run_debug_checks and settings.DEBUG:
                try:
                    ...
                    sample_products_data = list(
                        queryset.values("id", "product_name", "product_type")[:5]
                    )
                    prefetch_data = []
                    for p in sample_products_data:
                        try:
                            ...
                            prefetched_count = (
                                len(product_obj.available_units_list)
                            ...
                            prefetch_data.append({"product_id": p.get("id"), "error": str(e)})
                    with open("/tmp/affordable-gadgets-debug/debug.log", "a") as f:
                        f.write(
                            json.dumps(
                                {
            ...
            if brand and not is_detail:
                ...
                queryset = queryset.filter(Q(brands=brand) | Q(is_global=True)).distinct()
                            ...
                            json.dumps(
                                {
                                    ...
                                    "data": {"exists": after_brand_exists, "brand": str(brand)},
                                    "timestamp": int(time.time() * 1000),
                                }
                            )
                            + "\n"
                ...
                try:
                    if brand is None:
                        ...
                        promotion = Promotion.objects.get(
                            id=int(promotion_id), brand=brand, is_active=True
                        )
                        ...
                        if promotion.start_date <= now <= promotion.end_date:
                            ...
                            if promotion.products.exists():
                                ...
                                queryset = queryset.filter(
                                    id__in=promotion.products.values_list("id", flat=True)
                                )
            ...
            if settings.DEBUG:
                ...
                try:
                    ...
                    if final_count > 0:
                        ...
                        logger.warning(
                            "FINAL_COUNT_ZERO: No products returned! Initial queryset existed but all were filtered out."
                        )
                        ...
                        try:
                            ...
                            sample_products = list(
                                initial_queryset.values("id", "product_name")[:5]
                            )
                            for prod_data in sample_products:
                                ...
                                logger.error(
                                    f"[PRODUCT_DEBUG] Product {prod.id} ({prod.product_name}): {len(all_units)} units, statuses={unit_statuses}"
                                )
                        except Exception as e:
                                ...
                                sample_products.append(
                                    {
                                        ...
                                        "product_name": p["product_name"],
                                        "product_type": p["product_type"],
                                        "annotated_available_units_count": p[
                                            "available_units_count"
                                        ],
                                        "prefetched_units_count": prefetched_count,
    ...
    def units(self, request, pk=None):
        ...
        images_prefetch = Prefetch(
            "images",
            queryset=InventoryUnitImage.objects.select_related("color").order_by(
                "-is_primary", "id"
            ),
        )

        units = (
            product.inventory_units.filter(
                sale_status=InventoryUnit.SaleStatusChoices.AVAILABLE, available_online=True
            )
        ...
        serializer = PublicInventoryUnitSerializer(
            units, many=True, context={"request": request, "brand": brand}
        )
...
class CartViewSet(_PublicAPIMixin, _SilkProfileMixin, viewsets.ModelViewSet):
            ...
            return Response(
                {
                    ...
                    "detail": str(e)
                    if settings.DEBUG
                    else "An error occurred while creating the cart",
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def bundles(self, request, pk=None):
        ...
        if bundle_item_ids is not None and len(bundle_item_ids) == 0:
            return Response(
                {"error": "Select at least one bundle item"}, status=status.HTTP_400_BAD_REQUEST
            )

        try:
            bundle = Bundle.objects.get(id=bundle_id)
            created_items, group_id = CartService.add_bundle_to_cart(
                cart, bundle, main_inventory_unit_id=main_unit_id, bundle_item_ids=bundle_item_ids
            )
    ...
    def remove_item(self, request, pk=None, item_id=None):
        ...
        try:
            ...
            return Response(
                {"error": "Cart not found", "detail": str(e) if settings.DEBUG else None},
        ...
        try:
            ...
            if cart_item.quantity > 1:
                ...
                cart_item.save()
                logger.info(
                    f"Reduced quantity of item {item_id} in cart {cart.id} to {cart_item.quantity}"
                )
                serializer = CartItemSerializer(cart_item)
            ...
            return Response(
                {"error": "Cart item not found", "item_id": item_id, "cart_id": cart.id},
                status=status.HTTP_404_NOT_FOUND,
            )
        except Exception as e:
            logger.error(f"Error removing item: {str(e)}")
            return Response(
                {"error": str(e), "item_id": item_id, "cart_id": cart.id},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def checkout(self, request, pk=None):
        ...
        logger.info(
            f"Checkout request - Method: {request.method}, Content-Type: {request.content_type}"
        )
        ...
        try:
            cart = self.get_object()
            logger.info(
                f"Cart found: {cart.id}, brand: {cart.brand}, submitted: {cart.is_submitted}"
            )
        except Exception as e:
            logger.error(f"Cart lookup failed: {str(e)}")
            return Response(
        ...
        if cart.is_submitted:
            return Response(
                {"error": "Cart already submitted", "cart_id": cart.id},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if "delivery_address" in data:
            ...
            if address_value is None or (
                isinstance(address_value, str) and not address_value.strip()
            ):
        ...
        if not serializer.is_valid():
            ...
            return Response(
                {"error": "Validation failed", "errors": serializer.errors, "data_received": data},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"error": str(e), "cart_id": cart.id}, status=status.HTTP_400_BAD_REQUEST
            )
        except Exception as e:
            import traceback
            ...
            return Response(
                {
                    ...
                    if settings.DEBUG
                    else "An error occurred during checkout",
                    "cart_id": cart.id,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
            ...
            return Response(
                {
                    ...
                    "detail": str(e)
                    if settings.DEBUG
                    else "An error occurred while recognizing customer",
                    "customer": None,
            ...
            eligible_items.append(
                {
                    ...
                    "purchase_date": item.order.created_at.date()
...
class PublicWishlistViewSet(_PublicAPIMixin, _SilkProfileMixin, viewsets.ModelViewSet):
    ...
    def get_serializer_context(self):
        ...
        context["brand"] = getattr(self.request, "brand", None)
        context["view_action"] = (
            "list"  # Avoid N+1: nested PublicProductListSerializer returns None for unannotated fields
        )
        context["_image_url_cache"] = {}  # Reuse image URL building across wishlist items
        return context

    def _get_customer_and_session(self, request):
        customer_phone = request.data.get("customer_phone") or request.query_params.get(
            "customer_phone"
        )
        session_key = (
    ...
    def get_queryset(self):
        ...
        if brand:
            bundles_qs = bundles_qs.filter(brand=brand)
        bundles_prefetch = Prefetch(
            "product__bundles", queryset=bundles_qs, to_attr="active_bundles_list"
        )
        reviews_prefetch = Prefetch(
            "product__reviews",
            queryset=Review.objects.only("product_id", "rating"),
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not customer_phone and not session_key:
            return Response(
                {"detail": "customer_phone or session_key is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if customer_phone:
            customer, _ = CustomerService.get_or_create_customer(
                name="", phone=customer_phone, email=None, delivery_address=None
            )
...
class PublicBundleViewSet(_PublicAPIMixin, _SilkProfileMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        now = timezone.now()
        queryset = (
            Bundle.objects.filter(brand=brand, is_active=True)
