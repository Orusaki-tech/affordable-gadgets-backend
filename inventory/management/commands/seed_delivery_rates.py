...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=str(
                Path(settings.BASE_DIR) / "inventory" / "data" / "wards_nairobi_kiambu.csv"
            ),
    ...
    def handle(self, *args, **options):
        ...
        for county in COUNTIES:
            rate, created = DeliveryRate.objects.get_or_create(
                county=county, ward=None, defaults={"price": county_price, "is_active": True}
            )
            if created:
                created_counties += 1
        ...
        if not wards_file.exists():
            self.stdout.write(
                self.style.WARNING(f"Wards file not found at {wards_file}. Skipping ward seeding.")
            )
            return
        ...
        with wards_file.open(newline="", encoding="utf-8") as handle:
            ...
            for row in reader:
                ...
                if not county or not ward:
                    continue
                rate, created = DeliveryRate.objects.get_or_create(
                    county=county, ward=ward, defaults={"price": ward_price, "is_active": True}
                )
                if created:
                    created_wards += 1
                elif update_existing:
