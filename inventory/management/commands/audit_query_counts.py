    ...
    def handle(self, *args, **options):
        ...
        if use_auth:
            ...
            if admin and hasattr(admin.user, "auth_token"):
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "--auth: no staff user with token found; inventory endpoints may 401"
                    )
                )
        ...
        with override_settings(ALLOWED_HOSTS=allowed):
            for method, path in paths:
                with CaptureQueriesContext(connection) as ctx:
                    if method == "GET":
                        ...
                        response = client.post(
                            path, {}, content_type="application/json", **extra_headers
                        )
                ...
                style = (
                    self.style.ERROR
                    if num_queries > 50
                    else (self.style.WARNING if num_queries > 20 else self.style.SUCCESS)
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Total requests: {len(results)}  |  Total queries: {total}")
        )
        self.stdout.write(self.style.SUCCESS("=" * 72))
