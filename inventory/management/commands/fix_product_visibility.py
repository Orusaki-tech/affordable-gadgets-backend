    ...
    def handle(self, *args, **options):
        ...
        if product_id:
            products = Product.objects.filter(
                id=product_id, is_published=True, is_discontinued=False
            )
        else:
                ...
                for unit in units:
                    if unit.sale_status != InventoryUnit.SaleStatusChoices.AVAILABLE:
                        ...
                        units_to_fix.append(
                            {
                                "unit": unit,
                                "issue": "available_online is False (should be True)",
            ...
            if fix:
                ...
                for item in units_to_fix:
                    ...
                    if action == "set_status_available":
                        ...
                        self.stdout.write(
                            f"  ✓ Unit {unit.id}: Changed status from {old_status} to AVAILABLE"
                        )
                        fixed_count += 1
                ...
                self.stdout.write("   Run with --fix to actually update them.\n")
        else:
