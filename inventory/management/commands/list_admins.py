...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--create", action="store_true", help="Create a new admin user if none exist"
        )
    ...
    def handle(self, *args, **options):
        ...
        for idx, admin in enumerate(admins, 1):
            ...
            self.stdout.write(
                f"   Is Active: {self.style.SUCCESS('Yes' if user.is_active else 'No')}"
            )

            # Show password hint (can't show actual password)
            if user.has_usable_password():
                self.stdout.write(
                    f"   Password: {self.style.WARNING('*** (set) - Cannot display actual password')}"
                )
            else:
    ...
    def _create_default_admin(self):
        ...
        try:
            ...
            Admin.objects.create(user=user, admin_code=admin_code)
