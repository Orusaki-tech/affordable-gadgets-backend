    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING("   Admin Profile: MISSING"))
            ...
            if authenticated_user:
                ...
                self.stdout.write(
                    self.style.ERROR("❌ Django authentication FAILED - Invalid password")
                )
                return
            ...
            serializer_email = AdminAuthTokenSerializer(
                data={"username": email, "password": password}
            )

            if serializer_email.is_valid():
                self.stdout.write(
                    self.style.SUCCESS("✅ Serializer validation SUCCESS (with email)")
                )
                ...
                self.stdout.write(self.style.ERROR("❌ Serializer validation FAILED (with email)"))
                self.stdout.write(f"   Errors: {serializer_email.errors}")
                return

            # Test serializer with username
            serializer_username = AdminAuthTokenSerializer(
                data={"username": user.username, "password": password}
            )

            if serializer_username.is_valid():
                self.stdout.write(
                    self.style.SUCCESS("✅ Serializer validation SUCCESS (with username)")
                )
            else:
                self.stdout.write(
                    self.style.ERROR("❌ Serializer validation FAILED (with username)")
                )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING("⚠️  Token does not exist (will be created on login)")
                )
            ...
            if user.is_staff or user.is_superuser:
                self.stdout.write(self.style.SUCCESS("✅ User CAN login via AdminTokenLoginView"))
                self.stdout.write(
                    f"   Reason: User has is_staff={user.is_staff} or is_superuser={user.is_superuser}"
                )
            else:
                self.stdout.write(self.style.ERROR("❌ User CANNOT login via AdminTokenLoginView"))
                self.stdout.write("   Reason: User does not have is_staff=True")
                self.stdout.write("\n   FIX: Setting is_staff=True for this user...")
                user.is_staff = True
                user.save()
                self.stdout.write(self.style.SUCCESS("   ✅ Fixed! User can now login."))
