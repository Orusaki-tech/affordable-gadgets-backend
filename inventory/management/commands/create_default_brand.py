        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Could not create brand {brand_code}. It may already exist or there may be a database issue."
                        )
                    )
                    return
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f"Unexpected error checking for brand: {e}"))
                    return
                ...
                if created:
                    ...
                    if not brand.is_active:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f'Brand "{brand_code}" already exists and is active. Skipping creation.'
                            )
                        )
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Failed to create default brand: {e}"))
