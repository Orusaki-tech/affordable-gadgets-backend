...
from inventory.models import InventoryUnit, Order
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for order in paid_orders:
            ...
            if units_to_fix:
                ...
                for unit in units_to_fix:
                    self.stdout.write(
                        f"    - Unit {unit.id}: {unit.product_template.product_name if unit.product_template else 'N/A'}"
                    )
                    ...
                    if not dry_run:
                        with transaction.atomic():
                            ...
                            total_units_fixed += 1
                            self.stdout.write(self.style.SUCCESS("      ✓ Updated to SOLD"))
                    else:
                        self.stdout.write(self.style.WARNING("      → Would update to SOLD"))
                        total_units_fixed += 1
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(
                    f"DRY RUN: Would fix {total_units_fixed} units in {orders_processed} orders"
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Fixed {total_units_fixed} units in {orders_processed} orders"
                )
            )
