...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not orphaned_users:
            ...
            if email_filter:
                self.stdout.write(
                    f"   Email {email_filter} is either in use or belongs to a superuser."
                )
            return
        ...
        if not delete:
            ...
            self.stdout.write(
                self.style.WARNING("   Run with --delete to actually delete these users.")
            )
            if email_filter:
                self.stdout.write(f"\n   To delete the user with email {email_filter}, run:")
                self.stdout.write(
                    f"   python manage.py cleanup_orphaned_users --email {email_filter} --delete"
                )
            return
        ...
        try:
            ...
            self.stdout.write("\n" + "=" * 60)
            self.stdout.write(
                self.style.SUCCESS(f"✅ Successfully deleted {deleted_count} orphaned user(s)")
            )
