...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            ...
            if column_exists:
                self.stdout.write(self.style.SUCCESS("✅ Column idempotency_key already exists!"))
                return

            self.stdout.write("Column does not exist. Applying migration...")

            if dry_run:
                self.stdout.write(self.style.WARNING("DRY RUN: Would execute the following:"))
                self.stdout.write(
                    "  - ALTER TABLE inventory_order ADD COLUMN idempotency_key VARCHAR(255) NULL;"
                )
                self.stdout.write(
                    "  - CREATE UNIQUE INDEX inventory_order_idempotency_key_idx ON inventory_order(idempotency_key) WHERE idempotency_key IS NOT NULL;"
                )
                return
            ...
            try:
                ...
                try:
                    ...
                    self.stdout.write(
                        "  You may need to run: python manage.py migrate --fake inventory 0027"
                    )
