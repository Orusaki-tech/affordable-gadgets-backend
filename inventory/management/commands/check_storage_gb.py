    ...
    def handle(self, *args, **options):
        ...
        if product_id:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ Product {product_id} not found or not published")
                )
                return
        else:
        ...
        if units_with_storage.exists():
            ...
            for unit in units_with_storage[:10]:
                self.stdout.write(
                    f"    - Unit ID: {unit.id}, Storage: {unit.storage_gb}GB, Price: {unit.selling_price}, Color: {unit.color_name or 'N/A'}"
                )
        ...
        if units_without_storage.exists():
            ...
            for unit in units_without_storage[:10]:
                self.stdout.write(
                    f"    - Unit ID: {unit.id}, Price: {unit.selling_price}, Color: {unit.color_name or 'N/A'}"
                )
        ...
        serializer = PublicInventoryUnitSerializer(
            api_units, many=True, context={"request": request, "brand": None}
        )
        ...
        if api_units_with_storage:
            ...
            for unit in api_units_with_storage[:10]:
                self.stdout.write(
                    f"    - Unit ID: {unit.get('id')}, Storage: {unit.get('storage_gb')}GB, Price: {unit.get('selling_price')}, Color: {unit.get('color_name') or 'N/A'}"
                )
        ...
        if api_units_without_storage:
            ...
            for unit in api_units_without_storage[:10]:
                self.stdout.write(
                    f"    - Unit ID: {unit.get('id')}, Price: {unit.get('selling_price')}, Color: {unit.get('color_name') or 'N/A'}"
                )
                # Check if storage_gb field exists at all
                if "storage_gb" not in unit:
                    self.stdout.write(
                        self.style.WARNING(
                            "      ⚠️  WARNING: storage_gb field is MISSING from response!"
                        )
                    )
                elif unit.get("storage_gb") is None:
                    self.stdout.write("      ℹ️  storage_gb field exists but is NULL")
