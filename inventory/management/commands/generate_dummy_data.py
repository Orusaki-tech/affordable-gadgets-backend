...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self._create_unit_images(units, colors)
        self._create_accessories(products)
        self._create_reviews(products, customers, options["reviews"])
        promotions = self._create_promotions(
            brands, promotion_types, products, admins, options["promotions"]
        )
        ...
        self._create_orders(customers, brands, units, options["orders"])
        self._create_leads(customers, brands, units, admins, options["leads"])
        self._create_carts(customers, brands, units, promotions, options["carts"])
        self._create_notifications(
            admins, customers, reservations, returns, transfers, options["notifications"]
        )
        self._create_audit_logs(admins, customers, options["audit_logs"])
    ...
    def _ensure_brands(self):
        ...
        for brand_data in brands_data:
            brand, _ = Brand.objects.get_or_create(code=brand_data["code"], defaults=brand_data)
            brands.append(brand)
        return brands
    ...
    def _ensure_admin_roles(self):
        ...
        for role_code, role_label in [
            ...
            role, _ = AdminRole.objects.get_or_create(
                name=role_code,
                defaults={"display_name": role_label, "description": f"{role_label} role"},
            )
        ...
        for role_code, role_choice in role_mapping.items():
            ...
            for i in range(2 if role_code == "IM" else 3):
                ...
                admin, _ = Admin.objects.get_or_create(
                    user=user, defaults={"admin_code": f"{role_code}{i + 1}"}
                )
        ...
        types_data = [
            {
                "name": "Special Offer",
                "code": "SO",
                "description": "General special offers and discounts",
            },
            {
                "name": "Flash Sale",
                "code": "FS",
                "description": "Limited-time flash sales with urgent discounts",
            },
            {
                "name": "New Arrival",
                "code": "NA",
                "description": "Promotions for newly arrived products",
            },
            {"name": "Weekend Deal", "code": "WD", "description": "Special weekend promotions"},
            {
                "name": "Bundle Deal",
                "code": "BD",
                "description": "Bundle promotions for multiple products",
            },
            {
                "name": "Clearance Sale",
                "code": "CL",
                "description": "Clearance sales for discontinued or excess inventory",
            },
        ]
        types = []
    ...
    def _create_products(self, count, tags, brands, admins):
        ...
        for i in range(count):
            ...
            product = Product.objects.create(
                ...
                product_video_url="https://www.youtube.com/watch?v=dQw4w9WgXcQ"
                if (
                    (product_type == "AC" and random.choice([True, True, False, False, False]))
                    or (
                        product_type != "AC"
                        and (
                            (i < 15 and random.choice([True, True, False]))
                            or (i >= 15 and random.choice([True, False, False, False]))
                        )
                    )
                )
                else None,
            ...
            if admins:
                content_creators = [
                    a for a in admins if a.has_role(AdminRole.RoleChoices.CONTENT_CREATOR)
                ]
                creator = (
                    random.choice(content_creators) if content_creators else random.choice(admins)
                )
        ...
        for idx, product in enumerate(products):
            for unit_num in range(units_per_product):
                ...
                unit = InventoryUnit.objects.create(
                    ...
                    if product.product_type in ["PH", "LT", "TB"]
                    else None,
                    ram_gb=random.choice(ram_options)
                    if product.product_type in ["PH", "LT", "TB"]
                    else None,
                    battery_mah=random.choice(battery_options)
                    if product.product_type in ["PH", "TB"]
                    else None,
                    processor_details=random.choice(processors)
                    if product.product_type == "LT"
                    else "",
                    product_color=color,
                    ...
                    acquisition_source_details=random.choice(sources)
                    if sources and random.choice([True, False])
                    else None,
    ...
    def _create_product_images(self, products):
        ...
        for idx, product in enumerate(products):
            ...
            for img_num in range(num_images):
                try:
                    ...
                    img_file = self._download_banner_image(
                        img_url, f"product_{product.id}_{img_num + 1}.jpg"
                    )
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"  ⚠️  Could not create image for product {product.id}: {e}"
                        )
                    )
                    ...
                    try:
                        ...
                        img_file = self._download_banner_image(
                            alt_url, f"product_{product.id}_{img_num + 1}.jpg"
                        )
                        if img_file:
                            product_image = ProductImage.objects.create(
                                product=product,
        ...
        if product_images:
            self.stdout.write(
                f"  ✓ Created {len(product_images)} product images for {len(products)} products"
            )
        else:
        ...
        for idx, unit in enumerate(units_with_images):
            ...
            for img_num in range(num_images):
                try:
                    ...
                    img_file = self._download_banner_image(
                        img_url, f"unit_{unit.id}_{img_num + 1}.jpg"
                    )
                    ...
                    if img_file:
                        unit_image = InventoryUnitImage.objects.create(
                            ...
                            image=img_file,
                            is_primary=(img_num == 0),  # First image is primary
                            color=unit.product_color
                            if unit.product_color
                            else (random.choice(colors) if colors else None),
                        )
                        unit_images.append(unit_image)
                except Exception as e:
                    self.stdout.write(
                        self.style.WARNING(f"  ⚠️  Could not create image for unit {unit.id}: {e}")
                    )
                    continue
    ...
    def _create_accessories(self, products):
        ...
        if not all_products or not accessory_products:
            self.stdout.write(
                self.style.WARNING(
                    "  ⚠️  No accessories created: need both products and accessory products"
                )
            )
            return accessories
        ...
        for main_product in products_to_link:
            ...
            for accessory in selected_accessories:
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"  ⚠️  Could not link accessory {accessory.id} to product {main_product.id}: {e}"
                        )
                    )
    ...
    def _create_reviews(self, products, customers, count):
        ...
        for i in range(count):
            ...
            customer = (
                ...
                if customers and random.choice([True, True, False])
                else None
            )
            ...
            has_video = (
                ...
                if i < 20
                else random.choice([True, False, False, False, False])
            )
            ...
            review = Review.objects.create(
                ...
                video_url="https://www.youtube.com/watch?v=dQw4w9WgXcQ" if has_video else None,
            )
    ...
    def _download_banner_image(self, url, filename):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f"  ⚠️  Could not download banner image from {url}: {e}")
            )
            return None
    ...
    def _create_promotions(self, brands, promotion_types, products, admins, count):
        ...
        promotions = []
        marketing_managers = [
            a for a in admins if a.has_role(AdminRole.RoleChoices.MARKETING_MANAGER)
        ]
            ...
            for type_idx, promo_type in enumerate(promotion_types):
                ...
                for promo_idx in range(promotions_per_type):
                    ...
                    brand_products = [
                        ...
                        for p in products
                        if p.is_published
                        and (brand in p.brands.all() or (hasattr(p, "brand") and p.brand == brand))
                    ]
                    ...
                    if promo_type.code == "FS":  # Flash Sale
                        ...
                        display_locs = random.sample(
                            ["stories_carousel", "special_offers", "flash_sales"],
                            random.randint(1, 3),
                        )
                        discount_pct = (
                        ...
                        discount_amt = (
                            random.randint(5000, 20000) if random.choice([True, False]) else None
                        )
                    ...
                    if "stories_carousel" in display_locs:
                        ...
                        banner_image_file = self._download_banner_image(
                            banner_url,
                            f"promo_{promo_type.code.lower()}_{type_idx + 1}_{promo_idx + 1}.jpg",
                        )
                    ...
                    title_templates = {
                        "FS": [
                            ...
                            "⚡ Super Flash Sale - "
                            + random.choice(["Don't Miss Out", "Limited Stock", "Act Fast"]),
                        ],
                        "SO": [
                    ...
                    title_list = title_templates.get(
                        promo_type.code, [f"{promo_type.name} - Special Deal"]
                    )
                    ...
                    if promo_idx > 0:
                        ...
                        brand_products = [
                            ...
                            if p.is_published
                            and (
                                brand in p.brands.all()
                                or (hasattr(p, "brand") and p.brand == brand)
                            )
                        ]
                        if not brand_products:
                            brand_products = [p for p in products if p.is_published]
                ...
                promotion = Promotion.objects.create(
                    ...
                    product_types=random.choice(["PH", "LT", "TB", "AC", ""])
                    if random.choice([True, False])
                    else "",
                ...
                if brand_products:
                    selected_products = random.sample(
                        brand_products, min(num_products, len(brand_products))
                    )
                    # Ensure all selected products actually exist in the database
                    existing_product_ids = [
                        p.id for p in selected_products if Product.objects.filter(id=p.id).exists()
                    ]
                    if existing_product_ids:
                        promotion.products.set(existing_product_ids)
                        self.stdout.write(
                            f"    → Created {promo_type.name} promotion #{promo_idx + 1} with {len(existing_product_ids)} products (brand: {brand.name})"
                        )
                    else:
                        self.stdout.write(
                            self.style.WARNING(
                                f"    ⚠️  No valid products found for {promo_type.name} promotion #{promo_idx + 1}"
                            )
                        )
                else:
                    self.stdout.write(
                        self.style.WARNING(f"    ⚠️  No products available for brand {brand.name}")
                    )
        ...
        for i in range(remaining_count):
            ...
            display_locs = random.sample(
                ["stories_carousel", "special_offers", "flash_sales"], random.randint(1, 3)
            )
            ...
            if has_stories_carousel:
                ...
                banner_image_file = self._download_banner_image(
                    banner_url, f"promo_banner_{len(promotions) + i + 1}.jpg"
                )
            ...
            promotion = Promotion.objects.create(
                ...
                discount_amount=random.randint(5000, 20000)
                if random.choice([True, False])
                else None,
                start_date=start_date,
                end_date=end_date,
                ...
                display_locations=display_locs,
                product_types=random.choice(["PH", "LT", "TB", ""])
                if random.choice([True, False])
                else "",
                created_by=random.choice(marketing_managers) if marketing_managers else None,
                banner_image=banner_image_file if banner_image_file else None,
            ...
            if brand_products:
                ...
                if existing_product_ids:
                    ...
                    self.stdout.write(
                        self.style.WARNING("    ⚠️  No valid products found for promotion")
                    )
            else:
                self.stdout.write(
                    self.style.WARNING(f"    ⚠️  No products available for brand {brand.name}")
        ...
        self.stdout.write(
            f"  ✓ Created {len(promotions)} promotions (ensuring all {len(promotion_types) if promotion_types else 0} types are represented)"
        )
        self.stdout.write(
            f"  ✓ {promotions_with_products}/{len(promotions)} promotions have products associated ({total_products_linked} total product links)"
        )

        if promotions_with_products < len(promotions):
            self.stdout.write(
                self.style.WARNING(
                    f"  ⚠️  {len(promotions) - promotions_with_products} promotions have no products associated"
                )
            )
    ...
    def _create_reservation_requests(self, admins, units, count):
        ...
        inventory_managers = [
            a for a in admins if a.has_role(AdminRole.RoleChoices.INVENTORY_MANAGER)
        ]
        ...
        for i in range(count):
            ...
            units_to_reserve = random.sample(
                available_units, min(random.randint(1, 3), len(available_units))
            )
    ...
    def _create_return_requests(self, admins, units, count):
        ...
        for i in range(count):
            salesperson = random.choice(salespersons)
            units_to_return = random.sample(
                reserved_units, min(random.randint(1, 3), len(reserved_units))
            )
    ...
    def _create_leads(self, customers, brands, units, admins, count):
        ...
        for i in range(count):
            ...
            lead = Lead.objects.create(
                ...
                if customer
                else f"+2547{random.randint(10000000, 99999999)}",
                ...
                if salespersons and random.choice([True, False])
                else None,
                contacted_at=timezone.now() - timedelta(days=random.randint(1, 30))
                if status in ["CONTACTED", "CONVERTED"]
                else None,
                converted_at=timezone.now() - timedelta(days=random.randint(1, 20))
                if status == "CONVERTED"
                else None,
            ...
            for unit in selected_units:
                ...
                LeadItem.objects.create(
                    lead=lead, inventory_unit=unit, quantity=quantity, unit_price=unit_price
                )
                total += unit_price * quantity
        ...
        for i in range(count):
            ...
            for unit in selected_units:
                ...
                promotion = (
                    ...
                    if promotions and random.choice([True, False])
                    else None
                )
                unit_price = unit.selling_price
    ...
    def _create_notifications(self, admins, customers, reservations, returns, transfers, count):
        ...
        notifications = []
        all_users = list(User.objects.filter(admin__isnull=False)) + list(
            User.objects.filter(customer__isnull=False)
        )

        notification_types = ["RA", "RR", "RE", "TA", "TR", "FA", "FR", "OC", "UR", "RP", "NL"]
