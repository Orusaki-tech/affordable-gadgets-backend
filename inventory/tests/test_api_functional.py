...
class TestProductListingEndpoint:
    ...
    def test_products_list_returns_paginated_data(self, api_client, sample_product):
        ...
        assert 'results' in data or 'data' in data or isinstance(data, list), \
            "Response should contain results/data or be a list"
    ...
    def test_products_list_includes_published_only(self, api_client, db):
        ...
        published = baker.make(Product, is_published=True)
        unpublished = baker.make(Product, is_published=False)
        ...
        # Unpublished might be in results depending on permission model
    ...
    def test_products_list_supports_filtering(self, api_client, db):
        ...
        baker.make(Product, product_type=Product.ProductType.LAPTOP, is_published=True)
        
        # Test filter by product_type (if supported)
...
class TestProductDetailEndpoint:
    ...
    def test_product_detail_includes_article(self, api_client, sample_product, sample_article):
        ...
        data = response.json()
        # Article might be nested or in a separate endpoint
        assert 'id' in data
...
class TestProductArticleManagement:
    ...
    def test_create_article_requires_auth(self, api_client, sample_product):
        ...
        response = api_client.patch(f'/api/products/{sample_product.id}/update_content/', payload)
        
        # Either 401 (no auth) or 403 (forbidden) is expected
    ...
    def test_article_thumbnail_image_field_exists(self, api_client, sample_article):
        ...
        # Test via detail endpoint if available
        response = api_client.get(f'/api/products/{sample_article.product_id}/')
        
        if response.status_code == 200:
            data = response.json()
            # Thumbnail might be in article object or product
...
class TestContentCreatorPermissions:
    ...
    def test_content_creator_can_update_article(self, api_client, sample_product, content_creator_user, db):
        """Content creator should be able to update article fields."""
        api_client.force_authenticate(content_creator_user)
        
        payload = {
            'article': {
                'headline': 'Content Creator Update',
                'body': 'New content',
            }
        }
    ...
    def test_content_creator_cannot_modify_inventory(self, api_client, sample_product, content_creator_user):
        """Content creator should not be able to modify inventory fields."""
        api_client.force_authenticate(content_creator_user)
        
        payload = {
            'quantity': 1000,  # Should not be allowed
        }
        ...
        # Either forbidden or field is silently ignored
        assert response.status_code < 500
...
class TestAPIResponseFormats:
    ...
    def test_list_response_has_consistent_structure(self, api_client, db):
        ...
        baker.make(Product, is_published=True, _quantity=3)
        
        response = api_client.get('/api/products/')
        data = response.json()
        
        # Should be dict with results or list
    ...
    def test_error_responses_have_message(self, api_client):
        ...
        if response.status_code >= 400:
            data = response.json()
            # Error response should have some message/detail
            assert isinstance(data, (dict, list))
...
class TestMultipartFormDataHandling:
    ...
    def test_update_content_accepts_multipart_with_file(self, api_client, sample_product, admin_user):
        """update_content should accept multipart with file via __request."""
        api_client.force_authenticate(admin_user)
        
        # This test verifies the endpoint accepts multipart requests
        # (Actual file upload tested in integration/E2E tests)
