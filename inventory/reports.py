...
def get_inventory_value_report():
    ...
    total_value = InventoryUnit.objects.aggregate(total=Sum("selling_price"))["total"] or Decimal(
        "0"
    )

    # Value of available units only
    available_value = InventoryUnit.objects.filter(sale_status="AV").aggregate(
    ...
    by_product = (
        ...
        .annotate(
            ...
            available_count=Case(
                When(
                    product_template__product_type=Product.ProductType.ACCESSORY,
    ...
    # Value by status
    by_status = (
        InventoryUnit.objects.values("sale_status")
        .annotate(unit_count=Count("id"), total_value=Sum("selling_price"))
        .order_by("-total_value")
    )
...
def get_stock_movement_report(days=30):
    ...
    units_sourced = InventoryUnit.objects.filter(date_sourced__gte=start_date).count()

    # Units sold in period (approximated by status change)
    units_sold = InventoryUnit.objects.filter(sale_status="SD", updated_at__gte=start_date).count()

    # Daily breakdown
    daily_sourced = (
        InventoryUnit.objects.filter(date_sourced__gte=start_date)
        .annotate(date=TruncDate("date_sourced"))
        .values("date")
    ...
    daily_sold = (
        InventoryUnit.objects.filter(sale_status="SD", updated_at__gte=start_date)
        ...
        Product.objects.annotate(
            ...
            available_units=Case(
                When(
                    product_type=Product.ProductType.ACCESSORY,
                    then=Coalesce(
                        Sum("inventoryunit__quantity", filter=Q(inventoryunit__sale_status="AV")),
                        Value(0),
                    ),
                ),
            ...
            sold_units=Case(
                When(
                    product_type=Product.ProductType.ACCESSORY,
                    then=Coalesce(
                        Sum("inventoryunit__quantity", filter=Q(inventoryunit__sale_status="SD")),
                        Value(0),
                    ),
                ),
            ...
            reserved_units=Case(
                When(
                    product_type=Product.ProductType.ACCESSORY,
                    then=Coalesce(
                        Sum("inventoryunit__quantity", filter=Q(inventoryunit__sale_status="RS")),
                        Value(0),
                    ),
                ),
            ...
            total_revenue=Sum(
                "inventoryunit__selling_price", filter=Q(inventoryunit__sale_status="SD")
            ),
        )
        .annotate(
...
def get_salesperson_performance(days=30):
    ...
    for admin in salesperson_admins:
        ...
        reservations_requested = ReservationRequest.objects.filter(
            requesting_salesperson=admin, requested_at__gte=start_date
        ).count()

        reservations_approved = ReservationRequest.objects.filter(
            requesting_salesperson=admin, requested_at__gte=start_date, status="AP"
        ).count()

        approval_rate = (
            (reservations_approved / reservations_requested * 100)
            if reservations_requested > 0
            else 0
        )

        # Count returns
        returns_requested = ReturnRequest.objects.filter(
            requested_by=admin, requested_at__gte=start_date
        ).count()

        # Count transfers
        transfers_requested = UnitTransfer.objects.filter(
            Q(from_salesperson=admin) | Q(to_salesperson=admin), requested_at__gte=start_date
        ).count()
...
def get_request_management_stats(days=30):
    ...
    # Reservation stats
    reservation_stats = ReservationRequest.objects.filter(requested_at__gte=start_date).aggregate(
        total=Count("id"),
    ...
    # Return stats
    return_stats = ReturnRequest.objects.filter(requested_at__gte=start_date).aggregate(
        total=Count("id"),
        pending=Count("id", filter=Q(status="PE")),
        approved=Count("id", filter=Q(status="AP")),
        rejected=Count("id", filter=Q(status="RE")),
    ...
    # Transfer stats
    transfer_stats = UnitTransfer.objects.filter(requested_at__gte=start_date).aggregate(
        total=Count("id"),
        pending=Count("id", filter=Q(status="PE")),
        approved=Count("id", filter=Q(status="AP")),
        rejected=Count("id", filter=Q(status="RE")),
