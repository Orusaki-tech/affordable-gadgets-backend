...
# User is imported from .models above; use it for UserSerializer
...
class AdminSerializer(serializers.ModelSerializer):
    ...
    role_ids = serializers.PrimaryKeyRelatedField(
        queryset=AdminRole.objects.all(), source="roles", many=True, write_only=True, required=False
    )
    ...
    def get_reserved_units_count(self, obj):
        ...
        return InventoryUnit.objects.filter(
            reserved_by=obj, sale_status=InventoryUnit.SaleStatusChoices.RESERVED
        ).count()
        ...
        if existing_user:
            ...
            if Admin.objects.filter(user=existing_user).exists():
                raise serializers.ValidationError(
                    "This email address is already in use by an admin."
                )
        return value
...
class AdminAuthTokenSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not username_or_email:
            raise serializers.ValidationError(
                {"username": "Must include 'username' (can be username or email)."}
            )
...
class ProductImageSerializer(serializers.ModelSerializer):
    ...
    product = serializers.PrimaryKeyRelatedField(queryset=Product.objects.all(), read_only=False)
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    def get_seo_score(self, obj):
        ...
        if images_list and any(
            getattr(img, "alt_text", None) and str(img.alt_text).strip() for img in images_list
        ):
            score += 1
...
class ProductAccessorySerializer(serializers.ModelSerializer):
    ...
    accessory_video_url = serializers.CharField(
        source="accessory.product_video_url", read_only=True
    )
    ...
    def _pending_qty_for_unit(self, unit):
        ...
        if (
            not hasattr(unit, "_prefetched_objects_cache")
            or "order_items" not in unit._prefetched_objects_cache
        ):
    ...
    def get_accessory_primary_image(self, obj):
        ...
        primary_image = None
        if (
            hasattr(obj.accessory, "_prefetched_objects_cache")
            and "images" in obj.accessory._prefetched_objects_cache
        ):
            ...
            primary_image = ProductImage.objects.filter(
                product=obj.accessory, is_primary=True
            ).first()
            if not primary_image:
            ...
            if original_url.startswith("/media/") or original_url.startswith("/static/"):
                ...
                return (
                    ...
                    if (
                        ...
                        and "cloudinary.com" in cloudinary_url
                    )
                    else absolute_url
                )
            else:
    ...
    def get_accessory_color_variants(self, obj):
        ...
        color_variants = {}
        accessory_images = (
            list(obj.accessory.images.all())
            if hasattr(obj.accessory, "_prefetched_objects_cache")
            and "images" in obj.accessory._prefetched_objects_cache
            else []
        )
        ...
        for unit, available_qty in available_units:
            ...
            unit_images_list = (
                list(unit.images.all())
                if hasattr(unit, "_prefetched_objects_cache")
                and "images" in unit._prefetched_objects_cache
                else []
            )
            unit_image_obj = None
            ...
            if not unit_image_obj or not getattr(unit_image_obj, "image", None):
                ...
                if (
                    not unit_image_obj
                    and accessory_images
                    and getattr(accessory_images[0], "image", None)
                ):
            ...
            color_variants[color_id]["min_price"] = min(
                color_variants[color_id]["min_price"], price
            )
            color_variants[color_id]["max_price"] = max(
                color_variants[color_id]["max_price"], price
            )
...
class InventoryUnitSerializer(serializers.ModelSerializer):
    ...
    product_template = serializers.PrimaryKeyRelatedField(
        read_only=True
    )  # Read-only ID for filtering/matching
    product_template_name = serializers.CharField(
        source="product_template.product_name", read_only=True
    )
    ...
    acquisition_source_details_id = serializers.PrimaryKeyRelatedField(
        ...
        source="acquisition_source_details",
        allow_null=True,
        required=False,
        write_only=True,
    )
    ...
    def get_can_reserve(self, obj):
        ...
        try:
            ...
            return (
                admin.is_salesperson
                and obj.sale_status == InventoryUnit.SaleStatusChoices.AVAILABLE
            )
    ...
    def validate(self, data):
        ...
        current_source_details = data.get(
            "acquisition_source_details", getattr(self.instance, "acquisition_source_details", None)
        )
        ...
        if not is_accessory:
            if not current_sn:
                raise serializers.ValidationError(
                    {"serial_number": "Serial number is required for all unique products."}
                )

            if is_phone_or_tablet:
                if product_type == Product.ProductType.PHONE or (
                    product_type == Product.ProductType.TABLET and current_is_sim
                ):
                    if not current_imei:
                        raise serializers.ValidationError(
                            {"imei": "IMEI is required for Phones and SIM-enabled Tablets."}
                        )

                if not current_grade:
                    raise serializers.ValidationError(
                        {"grade": "Grade (A or B) is required for Phones and Tablets."}
                    )
            ...
            if is_laptop:
                ...
                if not data.get(
                    "processor_details", getattr(self.instance, "processor_details", None)
                ):
                    raise serializers.ValidationError(
                        {"processor_details": "Processor details are required for Laptops."}
                    )
        else:
        ...
        if not is_accessory:
            if brand == "apple":
                if not current_storage:
                    raise serializers.ValidationError(
                        {"storage_gb": "Storage is required for all Apple products."}
                    )
                if current_ram and current_ram != 0:
                    raise serializers.ValidationError(
                        {"ram_gb": "RAM should be blank for Apple products (storage-only input)."}
                    )
            else:  # All other brands require both RAM and Storage
                if not current_storage or not current_ram:
                    raise serializers.ValidationError(
                        {
                            "storage_gb": "Storage and RAM are required for non-Apple unique products."
                        }
                    )

        # --- 4. CONDITIONAL ACQUISITION SOURCE DETAILS ---
        if current_source in [
            InventoryUnit.SourceChoices.EXTERNAL_SUPPLIER,
            InventoryUnit.SourceChoices.EXTERNAL_IMPORT,
        ]:
            if not current_source_details:
                raise serializers.ValidationError(
                    {
                        "acquisition_source_details_id": "Acquisition source details are required for External Supplier or Import sources."
                    }
                )
    ...
    def to_representation(self, instance):
        ...
        if instance.review_image:
            ...
            optimized_url = get_optimized_image_url(
                instance.review_image, width=700, height=900, crop="fill"
            )
            if optimized_url:
    ...
    def get_review_image_url(self, obj):
        ...
        if obj.review_image:
            ...
            if "cloudinary.com" in original_url or "res.cloudinary.com" in original_url:
                cloudinary_url = get_optimized_image_url(
                    obj.review_image, width=700, height=900, crop="fill"
                )
                return cloudinary_url if cloudinary_url else original_url

            # If URL is local, try to construct Cloudinary URL from image name
            is_local_path = (
                original_url.startswith("/media/")
                or original_url.startswith("/static/")
                or "/media/" in original_url
                or "/static/" in original_url
            )
...
class ReservationRequestSerializer(serializers.ModelSerializer):
    ...
    requesting_salesperson_username = serializers.CharField(
        source="requesting_salesperson.user.username", read_only=True
    )
    ...
    class Meta:
        ...
        fields = (
            "id",
            "requesting_salesperson",
            "requesting_salesperson_username",
            "requesting_salesperson_brands",
            "inventory_unit",
            "inventory_unit_id",
            "inventory_unit_name",  # Legacy
            "inventory_units",
            "inventory_unit_ids",
            "inventory_unit_quantities",
            "inventory_units_details",  # New
            "status",
            "status_display",
            "requested_at",
            "approved_at",
    ...
    def get_inventory_units_details(self, obj):
        ...
        if not units and obj.inventory_unit:
            requested_quantity = (
                unit_quantities.get(str(obj.inventory_unit.id))
                or unit_quantities.get(obj.inventory_unit.id)
                or 1
            )
            units.append(
                {
    ...
    def validate_inventory_unit_ids(self, value):
        ...
        for unit in value:
            ...
            if unit.sale_status != InventoryUnit.SaleStatusChoices.AVAILABLE:
                raise serializers.ValidationError(
                    f"Unit {unit.id} is not available for reservation. Current status: {unit.get_sale_status_display()}"
                )
            # Only check reserved_by if unit is not already in this request
            if unit.reserved_by and unit.reserved_by != (
                current_request.requesting_salesperson if current_request else None
            ):
                raise serializers.ValidationError(
                    f"Unit {unit.id} is already reserved by another salesperson."
                )
        return value
    ...
    def validate(self, attrs):
        ...
        if self.instance:
            ...
            logger.info(
                f"Update validation for request {self.instance.id}. Attrs keys: {list(attrs.keys())}"
            )
            ...
            if unit_quantities is not None:
                ...
                if extra_ids:
                    raise serializers.ValidationError(
                        "Quantity provided for units not in this request."
                    )
                for unit in inventory_units:
                    requested_qty = normalized_quantities.get(unit.id)
                    if requested_qty is None:
                        raise serializers.ValidationError(
                            "Quantity must be provided for each unit in the request."
                        )
                    if (
                        unit.product_template.product_type != Product.ProductType.ACCESSORY
                        and requested_qty != 1
                    ):
                        raise serializers.ValidationError(
                            f"Unit {unit.id} does not support quantity > 1."
                        )
                    if (
                        unit.product_template.product_type == Product.ProductType.ACCESSORY
                        and requested_qty > unit.quantity
                    ):
                        raise serializers.ValidationError(
                            f"Unit {unit.id} only has {unit.quantity} available."
                        )
    ...
    def create(self, validated_data):
        ...
        reservation_request.inventory_unit_quantities = inventory_unit_quantities or {
            unit.id: 1 for unit in inventory_units
        }
        ...
        try:
            ...
            first_unit = (
                reservation_request.inventory_units.first() or reservation_request.inventory_unit
            )
            if not first_unit:
                return reservation_request

            salesperson = reservation_request.requesting_salesperson
            unit_count = reservation_request.inventory_units.count() or (
                1 if reservation_request.inventory_unit else 0
            )
            ...
            logger.info(
                f"Creating notifications for reservation request {reservation_request.id}. Found {manager_count} inventory managers."
            )

            if manager_count == 0:
                logger.warning(
                    f"No inventory managers found to notify for reservation request {reservation_request.id}"
                )
            ...
            for manager in managers:
                try:
                    ...
                    logger.info(
                        f"Notification created for inventory manager: {manager.user.username}"
                    )
                except Exception as e:
                    # Log error but don't break the request creation
                    logger.error(
                        f"Failed to create notification for manager {manager.user.username}: {e}",
                        exc_info=True,
                    )
            ...
            if superuser_count == 0:
                logger.warning(
                    f"No superusers found to notify for reservation request {reservation_request.id}"
                )
            ...
            for superuser in superusers:
                try:
                    ...
                    # Log error but don't break the request creation
                    logger.error(
                        f"Failed to create notification for superuser {superuser.username}: {e}",
                        exc_info=True,
                    )
        except Exception as e:
            # Log error but don't break the request creation
            logger.error(
                f"Failed to create notifications for reservation request {reservation_request.id}: {e}",
                exc_info=True,
            )
    ...
    def update(self, instance, validated_data):
        ...
        try:
            logger.info(
                f"Updating reservation request {instance.id}. Status: {instance.status}, Validated data keys: {list(validated_data.keys())}"
            )
            ...
            logger.info(
                f"Extracted units: inventory_units={inventory_units is not None}, inventory_unit={inventory_unit is not None}"
            )
            ...
            if inventory_units is not None:
                ...
                for unit in inventory_units:
                    logger.info(
                        f"Validating unit {unit.id}, status: {unit.sale_status}, reserved_by: {unit.reserved_by}, requesting_salesperson: {instance.requesting_salesperson.id if instance.requesting_salesperson else None}"
                    )
                    ...
                    if unit.id in current_unit_ids:
                        ...
                        if unit.reserved_by == instance.requesting_salesperson:
                            logger.info(
                                f"Unit {unit.id} is already in request and reserved by same salesperson, allowing"
                            )
                            continue
                        else:
                            # Unit is in request but reserved by someone else - this shouldn't happen, but handle it
                            logger.warning(
                                f"Unit {unit.id} is in request but reserved by different salesperson"
                            )
                            continue
            ...
            if inventory_unit:
                if inventory_unit.sale_status != InventoryUnit.SaleStatusChoices.AVAILABLE:
                    raise serializers.ValidationError(
                        f"Unit {inventory_unit.id} is not available for reservation."
                    )
            ...
            logger.error(
                f"Error updating reservation request {instance.id}: {str(e)}\n{traceback.format_exc()}"
            )
            raise
...
class ReturnRequestSerializer(serializers.ModelSerializer):
    ...
    def get_net_holdings_info(self, obj):
        ...
        if (
            obj.requesting_salesperson_id
            and obj.requesting_salesperson_id in net_holdings_by_admin_id
        ):
        ...
        if not salesperson:
            return {
                "directly_reserved": 0,
                "received_via_transfer": 0,
                "transferred_out": 0,
                "net_holdings": 0,
            }
        directly_reserved = InventoryUnit.objects.filter(
            reserved_by=salesperson, sale_status=InventoryUnit.SaleStatusChoices.RESERVED
        ).count()
        received_via_transfer = UnitTransfer.objects.filter(
            to_salesperson=salesperson, status=UnitTransfer.StatusChoices.APPROVED
        ).count()
        transferred_out = UnitTransfer.objects.filter(
            from_salesperson=salesperson, status=UnitTransfer.StatusChoices.APPROVED
        ).count()
        return {
        ...
        if self.instance is None:  # Creating new return request
            ...
            if not inventory_units:
                reserved_count = InventoryUnit.objects.filter(
                    reserved_by=admin, sale_status=InventoryUnit.SaleStatusChoices.RESERVED
                ).count()
                if reserved_count == 0:
                    raise serializers.ValidationError(
                        {
...
class UnitTransferSerializer(serializers.ModelSerializer):
    ...
    from_salesperson_username = serializers.CharField(
        source="from_salesperson.user.username", read_only=True
    )
    to_salesperson_username = serializers.CharField(
        source="to_salesperson.user.username", read_only=True
    )
    approved_by_username = serializers.CharField(source="approved_by.user.username", read_only=True)
    status_display = serializers.CharField(source="get_status_display", read_only=True)
    inventory_unit_name = serializers.CharField(
        source="inventory_unit.product_template.product_name", read_only=True
    )
    inventory_unit_id = serializers.PrimaryKeyRelatedField(
    ...
    class Meta:
        ...
        fields = (
            ...
            "inventory_unit_name",
            "from_salesperson",
            "from_salesperson_username",
            "to_salesperson",
            "to_salesperson_id",
            "to_salesperson_username",
            "status",
            "status_display",
            "requested_at",
            "approved_at",
    ...
    def validate(self, data):
        ...
        inventory_unit = (
            data.get("inventory_unit") or self.instance.inventory_unit if self.instance else None
        )
...
class LeadSerializer(serializers.ModelSerializer):
    ...
    assigned_salesperson_name = serializers.CharField(
        source="assigned_salesperson.user.username", read_only=True, allow_null=True
    )
    ...
    class Meta:
        ...
        read_only_fields = ("lead_reference", "submitted_at", "converted_at", "order_id")
    ...
    def get_customer_name_display(self, obj):
        ...
        if obj.customer:
            return obj.customer.name or (
                obj.customer.user.username if obj.customer.user else "Unknown"
            )
...
class PromotionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.banner_image:
            ...
            optimized_url = get_promotion_image_url(instance.banner_image, size="lg", crop="fill")
            if optimized_url:
    ...
    def get_product_count(self, obj):
        ...
        if obj.product_types:
            ...
            type_products = Product.objects.filter(product_type=obj.product_types, brands=obj.brand)
