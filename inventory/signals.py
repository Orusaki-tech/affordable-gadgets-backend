...
def handle_order_creation(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        inventory_managers = Admin.objects.filter(roles__name="IM").select_related("user")
    ...
    if created:
        ...
        if old_instance:
            ...
            if old_instance.selling_price != instance.selling_price:
                ...
                old_data["selling_price"] = (
                    str(old_instance.selling_price) if old_instance.selling_price else None
                )
                new_data["selling_price"] = (
                    str(instance.selling_price) if instance.selling_price else None
                )
            ...
            if changes:
                action = (
                    AuditLog.ActionType.PRICE_CHANGE
                    if "selling_price" in changes
                    else AuditLog.ActionType.STATUS_CHANGE
                )
                if len(changes) > 1:
...
def audit_log_reservation_request(sender, instance, created, **kwargs):
    ...
    if not created and instance.status in [
        ReservationRequest.StatusChoices.APPROVED,
        ReservationRequest.StatusChoices.REJECTED,
    ]:
        action = (
            AuditLog.ActionType.APPROVE
            if instance.status == ReservationRequest.StatusChoices.APPROVED
        ...
        if instance.approved_by:
            approved_by_user = (
                ...
                if hasattr(instance.approved_by, "user")
                else instance.approved_by
            )
...
def audit_log_return_request(sender, instance, created, **kwargs):
    ...
    if not created and instance.status in [
        ReturnRequest.StatusChoices.APPROVED,
        ReturnRequest.StatusChoices.REJECTED,
    ]:
        action = (
            AuditLog.ActionType.APPROVE
            if instance.status == ReturnRequest.StatusChoices.APPROVED
            else AuditLog.ActionType.REJECT
        )
...
def audit_log_unit_transfer(sender, instance, created, **kwargs):
    ...
    if not created and instance.status in [
        UnitTransfer.StatusChoices.APPROVED,
        UnitTransfer.StatusChoices.REJECTED,
    ]:
        action = (
            AuditLog.ActionType.APPROVE
            if instance.status == UnitTransfer.StatusChoices.APPROVED
            else AuditLog.ActionType.REJECT
        )
        ...
        AuditLog.objects.create(
            ...
            new_value={
                "status": instance.status,
                "from_salesperson": instance.from_salesperson.user.username
                if instance.from_salesperson
                else None,
                "to_salesperson": instance.to_salesperson.user.username
                if instance.to_salesperson
                else None,
            },
