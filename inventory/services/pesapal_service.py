...
class PesapalService:
    ...
    def __init__(self):
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {
                                ...
                                "consumer_key_first_10": self.consumer_key[:10]
                                if self.consumer_key
                                else None,
                                "consumer_key_full": str(self.consumer_key)
                                if self.consumer_key
                                else None,
                                ...
                                "raw_key_from_settings": str(settings.PESAPAL_CONSUMER_KEY)[:20]
                                if hasattr(settings, "PESAPAL_CONSUMER_KEY")
                                else None,
                                "raw_secret_from_settings": str(settings.PESAPAL_CONSUMER_SECRET)[
                                    :20
                                ]
                                if hasattr(settings, "PESAPAL_CONSUMER_SECRET")
                                else None,
                            },
                            "timestamp": int(timezone.now().timestamp() * 1000),
                        }
        ...
        print(
            f"[PESAPAL] Service initialized - Base URL: {self.base_url}, Environment: {self.environment}"
        )
        print(
            f"[PESAPAL] Consumer Key loaded: {self.consumer_key[:15] if self.consumer_key else 'None'}... (length: {len(self.consumer_key) if self.consumer_key else 0})"
        )
        print(
            f"[PESAPAL] Consumer Secret loaded: {'*' * 15}... (length: {len(self.consumer_secret) if self.consumer_secret else 0})"
        )
    ...
    def _make_request_with_failover(
        ...
        for base_url in urls_to_try:
            ...
            try:
                                ...
                                json.dumps(
                                    {
                                        ...
                                        "data": {
                                            ...
                                            "consumer_secret_in_data": "consumer_secret"
                                            in (data or {}),
                                            "headers": {
                ...
                if method.upper() == "POST":
                    print("[PESAPAL] Making POST request...")
                    # #region agent log
                    try:
                        with open(log_path, "a") as f:
                            f.write(
                                json.dumps(
                                    {
                                        ...
                                        "data": {
                                            ...
                                            "consumer_key_in_data": data.get("consumer_key")
                                            if data and isinstance(data, dict)
                                            else None,
                                            "consumer_key_is_none": data.get("consumer_key") is None
                                            if data and isinstance(data, dict)
                                            else None,
                                            "consumer_key_length": len(data.get("consumer_key", ""))
                                            if data
                                            and isinstance(data, dict)
                                            and data.get("consumer_key")
                                            else 0,
                                            "consumer_secret_in_data": "***"
                                            if data
                                            and isinstance(data, dict)
                                            and data.get("consumer_secret")
                                            else None,
                                            "consumer_secret_is_none": data.get("consumer_secret")
                                            is None
                                            if data and isinstance(data, dict)
                                            else None,
                    ...
                    print("[PESAPAL] Making GET request...")
                ...
                try:
                    response_text_preview = (
                        response.text[:1000] if hasattr(response, "text") else None
                    )
                    response_json = None
                    try:
                            ...
                            json.dumps(
                                {
                                    ...
                                    "data": {
                                        ...
                                        "error_in_response": response_json.get("error")
                ...
                if response.status_code in [200, 201]:
                    try:
                        ...
                        print(
                            f"[PESAPAL] SUCCESS - Response Body: {json.dumps(response_json, indent=2)}"
                        )
                    except Exception:
                        print(
                            f"[PESAPAL] SUCCESS - Response Text (first 500 chars): {response.text[:500]}"
                        )
                    print("[PESAPAL] ========== API REQUEST SUCCESS ==========\n")
                ...
                try:
                    ...
                    print(
                        f"[PESAPAL] ERROR - Full Response JSON: {json.dumps(error_data, indent=2)}"
                    )
                    ...
                    if (
                        "Authentication credentials were not provided" in error_str
                        or "credentials were not provided" in error_str.lower()
                    ):
                        print("[PESAPAL] CRITICAL: Pesapal says credentials not provided!")
                        print(
                            f"[PESAPAL] Request data that was sent: consumer_key present={bool(data and data.get('consumer_key'))}, consumer_secret present={bool(data and data.get('consumer_secret'))}"
                        )
                        if data:
                            print(f"[PESAPAL] Data keys: {list(data.keys())}")
                            print(
                                f"[PESAPAL] Consumer key type: {type(data.get('consumer_key'))}, length: {len(str(data.get('consumer_key', '')))}"
                            )
                    ...
                    logger.error(
                        f"Could not parse error response as JSON: {json_error}. Response text: {response.text[:500]}"
                    )

                if response.status_code in [401, 403]:
                    print(
                        f"[PESAPAL] AUTHENTICATION ERROR ({response.status_code}): {error_message}"
                    )
                    if retry_count < self.max_retries:
                        print(
                            f"[PESAPAL] Retrying with fresh token... (retry {retry_count + 1}/{self.max_retries})"
                        )
                        logger.warning(
                            f"Authentication error ({response.status_code}): {error_message}, retrying with fresh token..."
                        )
                        self._access_token = None
                        ...
                        detailed_error = (
                            f"Authentication failed ({response.status_code}): {error_message}"
                        )
                        print("[PESAPAL] ========== API REQUEST FAILED ==========")
                ...
                print("[PESAPAL] ========================================\n")
                logger.warning(
                    f"Request to {url} failed with status {response.status_code}: {error_message}"
                )
    ...
    def get_access_token(self, force_refresh: bool = False) -> str | None:
        ...
        if not self.consumer_key or not self.consumer_secret:
            print("[PESAPAL] ERROR: Credentials not configured!")
        ...
        if not force_refresh and self._access_token and self._token_expires_at:
            if timezone.now() < self._token_expires_at:
                ...
                print("[PESAPAL] ========== GET ACCESS TOKEN SUCCESS (CACHED) ==========\n")
        ...
        print(
            f"[PESAPAL] Consumer Key: {self.consumer_key[:15]}... (length: {len(self.consumer_key)})"
        )
        ...
        logger.debug(
            f"Consumer Key: {self.consumer_key[:10]}... (length: {len(self.consumer_key)})"
        )
        ...
        if error or not response:
            ...
            print("[PESAPAL] ========== GET ACCESS TOKEN FAILED ==========")
            ...
            if "Authentication" in error_msg or "401" in error_msg or "403" in error_msg:
                print("[PESAPAL] This usually means:")
                print("[PESAPAL]   1) Credentials are incorrect")
                print("[PESAPAL]   2) Credentials need activation in Pesapal dashboard")
                print("[PESAPAL]   3) Credentials are for a different environment")
                logger.error(
                    "This usually means: 1) Credentials are incorrect, 2) Credentials need activation in Pesapal dashboard, or 3) Credentials are for a different environment"
                )
            print("[PESAPAL] =============================================\n")
            return None
        ...
        try:
            ...
            if not result:
                print("[PESAPAL] ERROR: Empty response from Pesapal API")
            ...
            if error_info is not None:
                ...
                if has_actual_error:
                    if isinstance(error_info, dict):
                        error_message = (
                            error_info.get("message")
                            or error_info.get("error_type")
                            or error_info.get("error_description")
                            or "Unknown error"
                        )
                    else:
            ...
            if not token:
                print("[PESAPAL] ========== GET ACCESS TOKEN FAILED ==========")
                print("[PESAPAL] ERROR: No token in response")
            ...
            print("[PESAPAL] ========== GET ACCESS TOKEN SUCCESS ==========")
    ...
    def submit_order_request(self, order_data: dict) -> tuple[dict | None, str | None]:
        ...
        if not self.consumer_key or not self.consumer_secret:
            ...
            return (
                None,
                "Pesapal credentials not configured. Please check PESAPAL_CONSUMER_KEY and PESAPAL_CONSUMER_SECRET in settings.",
            )

        print("[PESAPAL] Getting access token...")
        token = self.get_access_token()
        if not token:
            print("[PESAPAL] ========== SUBMIT ORDER REQUEST FAILED ==========")
            print("[PESAPAL] ERROR: Failed to obtain access token")
            print("[PESAPAL] ================================================\n")
            return (
                None,
                "Failed to obtain access token from Pesapal. Please verify your credentials are correct and active.",
            )
        ...
        print("[PESAPAL] Submitting order request to Pesapal...")
        ...
        try:
            ...
            if (
                ...
                and "error" in result.get("message", "").lower()
            ):
                print("[PESAPAL] ========== SUBMIT ORDER REQUEST FAILED ==========")
            ...
            print("[PESAPAL] ========== SUBMIT ORDER REQUEST SUCCESS ==========")
    ...
    def get_transaction_status(self, order_tracking_id: str) -> tuple[dict | None, str | None]:
        ...
        token = self.get_access_token()
        if not token:
            print("[PESAPAL] ========== GET TRANSACTION STATUS FAILED ==========")
            print("[PESAPAL] ERROR: Failed to obtain access token")
            print("[PESAPAL] ===================================================\n")
        ...
        print("[PESAPAL] Requesting transaction status from Pesapal...")
        ...
        try:
            ...
            print("[PESAPAL] ========== GET TRANSACTION STATUS SUCCESS ==========")
            print("[PESAPAL] Status retrieved successfully")
            print("[PESAPAL] ===================================================\n")
            return result, None
    ...
    def register_ipn_url(
        ...
        token = self.get_access_token()
        if not token:
            print("[PESAPAL] ========== REGISTER IPN URL FAILED ==========")
            print("[PESAPAL] ERROR: Failed to obtain access token")
            print("[PESAPAL] ============================================\n")
            return None, "Failed to obtain access token"

        endpoint = "/api/URLSetup/RegisterIPN"
        data = {"url": ipn_url, "ipn_notification_type": ipn_notification_type}
        ...
        print("[PESAPAL] Registering IPN URL with Pesapal...")
        response, error = self._make_request_with_failover("POST", endpoint, data, headers)
        ...
        try:
            ...
            if not ipn_id:
                print("[PESAPAL] ========== REGISTER IPN URL FAILED ==========")
                print("[PESAPAL] ERROR: No ipn_id in response")
                print(f"[PESAPAL] Response: {json.dumps(result, indent=2)}")
                print("[PESAPAL] ============================================\n")
                return None, "No ipn_id in Pesapal API response"

            print("[PESAPAL] ========== REGISTER IPN URL SUCCESS ==========")
