...
class ReceiptService:
    ...
    def get_receipt_context(order: Order) -> dict:
        ...
        context = {
            ...
            "phone_number": "+254142530781",  # Company phone
    ...
    def generate_receipt_html(order: Order) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error generating receipt HTML for order {order.order_id}: {e}", exc_info=True
            )
            raise

    @staticmethod
    def generate_receipt_pdf(order: Order, html_content: str | None = None) -> bytes:
        ...
        try:
            ...
            logger.error(
                f"Error generating receipt PDF for order {order.order_id}: {e}", exc_info=True
            )
            raise
    ...
    @staticmethod
    def send_receipt_email(order: Order, receipt: Receipt | None = None) -> bool:
...
"""
            ...
            logger.error(
                f"Error sending receipt email for order {order.order_id}: {e}", exc_info=True
            )
            return False

    @staticmethod
    def send_receipt_whatsapp(order: Order, receipt: Receipt | None = None) -> bool:
        ...
        try:
            ...
            if not customer_phone:
                logger.warning(
                    f"No phone number found for order {order.order_id}, cannot send WhatsApp receipt"
                )
                return False
            ...
            logger.error(
                f"Error sending receipt WhatsApp for order {order.order_id}: {e}", exc_info=True
            )
            return False
    ...
    def generate_and_send_receipt(order: Order) -> tuple[Receipt, bool, bool]:
        ...
        try:
            ...
            logger.error(
                f"Error in generate_and_send_receipt for order {order.order_id}: {e}", exc_info=True
            )
            raise
