...
from inventory.models import Admin, AdminRole, InventoryUnit, Lead, Order, OrderItem
...
class LeadService:
    ...
    def auto_assign_lead(lead):
        ...
        salespersons = Admin.objects.filter(
            roles__name=AdminRole.RoleChoices.SALESPERSON, brands=lead.brand
        ).distinct()
    ...
    def convert_lead_to_order(lead, salesperson):
        ...
        with transaction.atomic():
            ...
            from inventory.observability import LEADS_CONVERTED, NEW_ORDERS_TOTAL
            ...
            try:
                ...
                if cart:
                    cart.delete()
                    print(
                        f"Cart {cart.id} deleted after lead {lead.lead_reference} converted to order"
                    )
