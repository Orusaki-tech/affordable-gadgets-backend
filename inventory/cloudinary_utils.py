...
def _get_cloudinary_url_from_field(image_field):
    ...
    if base_url.startswith("/media/") or base_url.startswith("/static/"):
        ...
        if request:
            ...
            protocol = (
                "https" if os.environ.get("DJANGO_USE_HTTPS", "").lower() == "true" else "http"
            )
    ...
    if hasattr(image_field, "name") and image_field.name:
        ...
        try:
            ...
            logger.warning(
                f"Failed to build Cloudinary URL from public_id '{image_field.name}': {e}"
            )
...
def get_optimized_image_url(
    image_field, width=None, height=None, quality="auto", format="auto", crop="fill"
):
    """
    ...
    if hasattr(image_field, "name") and image_field.name:
        try:
            ...
            logger.warning(
                f"Failed to build Cloudinary URL from image_field.name '{image_field.name}': {e}"
            )
    ...
    if base_url and ("cloudinary.com" in base_url or "res.cloudinary.com" in base_url):
        ...
        try:
            ...
            if "/upload/" in base_url:
                ...
                if len(parts) == 2:
                    ...
                    if transformations:
                        ...
                        if "," in after_upload.split("/")[0] or any(
                            x in after_upload.split("/")[0] for x in ["w_", "h_", "c_", "q_", "f_"]
                        ):
            ...
            logger.warning(
                f"Failed to add transformations to existing Cloudinary URL: {e}. Reconstructing..."
            )
    ...
    try:
        ...
        try:
            ...
            if (
                not public_id.startswith("media/")
                and hasattr(image_field, "name")
                and image_field.name
                and "media/" in image_field.name
            ):
            ...
            logger.warning(
                f"Failed to parse Cloudinary URL '{base_url}': {e}. Using string replacement method."
            )
        ...
        logger.warning(
            f"Failed to build Cloudinary URL with transformations: {e}. Using string replacement method."
        )
        # Fallback to string replacement method
...
def get_thumbnail_url(image_field, size=200):
    ...
    return get_optimized_image_url(
        image_field, width=size, height=size, crop="fill", quality="auto", format="auto"
    )
...
def get_product_image_url(image_field, size="medium"):
    ...
    if size in size_map:
        ...
        return get_optimized_image_url(
            image_field, width=width, height=height, crop="fill", quality="auto", format="auto"
        )

    # Default to medium
    return get_optimized_image_url(
        image_field, width=800, height=800, crop="fill", quality="auto", format="auto"
    )
...
def get_promotion_image_url(image_field, size="lg", crop="fill"):
    ...
    return get_optimized_image_url(
        image_field, width=size_px, height=size_px, crop=crop, quality="auto", format="auto"
    )
