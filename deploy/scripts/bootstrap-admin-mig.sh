#!/usr/bin/env bash
# Bootstrap admin MIG instances: Docker + GCS compose + up (until startup scripts are reliable).
set -euo pipefail

PROJECT="${GCP_PROJECT_ID:-project-07850c05-c54d-486b-80a}"
ZONE="${GCP_ZONE:-us-central1-a}"
MIG="${ADMIN_MIG_NAME:-affordable-gadgets-staging-admin-mig}"
...
sudo gsutil -q cp gs://${BUCKET}/staging/admin/docker-compose.admin.yml /opt/affordable-gadgets/
cd /opt/affordable-gadgets
sudo docker rm -f ag-admin 2>/dev/null || true
sudo docker-compose -f docker-compose.admin.yml pull -q
sudo docker-compose -f docker-compose.admin.yml up -d
sleep 10
curl -sf http://127.0.0.1:80/health >/dev/null && echo ${INSTANCE}_OK || curl -sf http://127.0.0.1:80/ >/dev/null && echo ${INSTANCE}_OK || echo ${INSTANCE}_FAIL
" || echo "WARN: bootstrap failed for ${INSTANCE}"
done

ADMIN_LB="$(cd "$(dirname "$0")/../terraform" && terraform output -raw admin_lb_ip 2>/dev/null || true)"
if [[ -n "${ADMIN_LB}" ]]; then
  echo "==> Admin LB:"
  curl -sf --max-time 20 "http://${ADMIN_LB}/" >/dev/null && echo OK || echo "not ready"
fi
