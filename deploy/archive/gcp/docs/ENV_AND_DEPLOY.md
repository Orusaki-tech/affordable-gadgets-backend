...
| **PESAPAL_IPN_URL** | Often a stale Render URL in `.env` | **deploy-gcp.sh** overwrites with `https://<ngrok-host>/api/inventory/pesapal/ipn/` when it can read the live tunnel from the VM (`127.0.0.1:4040`). **ngrok-on-vm.sh** always writes this after each tunnel URL. Required for M-Pesa initiation. |
