                    ...
                    for promotion in data['results']:
                        ...
                        if banner_image:
                            ...
                            if '.auto' in banner_image:
                                print(f"   ⚠️  URL still has '.auto' extension (fix may not be deployed yet)")
                                all_working = False
                            else:
                                print(f"   ✅ URL does NOT have '.auto' extension")
                            ...
                            if result['accessible']:
                                ...
                                print(f"   ❌ Not accessible: HTTP {result.get('status', result.get('error'))}")
                                all_working = False
                                ...
                                if '.auto' in banner_image:
                                    ...
                                    if result2['accessible']:
                                        print(f"   ✅ Works without .auto! (Status: {result2['status']})")
                                        print(f"   💡 Fix needs to be deployed to remove .auto from URLs")
                        else:
                            print(f"\n   ❌ No banner image URL")
                            all_working = False
